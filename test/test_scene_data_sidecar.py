...
try:
    ...
    check(
        "manifest_path_for names the sidecar",
        os.path.basename(SD.manifest_path_for(export)) == ".shot_v003.scene_data.json",
    )
    check("base_stem strips _vNN", SD.base_stem(export) == "shot")
    check(
        "base_stem manifest shares across versions",
        os.path.basename(SD.manifest_path_for(export, base_stem=True))
        == ".shot.scene_data.json",
    )
    check(
        "base_stem doesn't strip mid-name v",
        SD.base_stem(os.path.join(tmp, "arch_v2_proxy.fbx")) == "arch_v2_proxy",
    )
    ...
    check(
        "manifest is format 3 with a hierarchy section",
        raw.get("format") == 3
        and raw["hierarchy"]["object_count"] == 3
        and bool(raw["hierarchy"]["hash"]),
    )
    check(
        "empty data section is omitted", "data_export" not in raw and "paths" not in raw
    )
    ...
    check(
        "hierarchy read unaffected by data section", SD.read_manifest(export) == paths_a
    )
    ...
    check(
        "data-only change keeps hierarchy compare matching",
        SD.compare(export, paths_a) == (True, [], []),
    )
    ...
    check(
        "authoring locate hint is not recorded",
        "Dropbox (Client)" not in hint_raw
        and "dir" not in written["lightmap_metadata"],
    )
    check(
        "scrub keeps the payload and sibling channels",
    ...
    with open(SD.manifest_path_for(v1_export), "w") as f:
        json.dump(
            {"paths": v1_paths, "object_count": 2, "hash": SD._paths_hash(v1_paths)}, f
        )
    check(
        "v1 flat manifest reads as hierarchy section",
        SD.read_manifest(v1_export) == set(v1_paths),
    )
    check("v1 flat manifest has no data section", SD.read_data(v1_export) is None)
    check(
        "v1 flat manifest compares via hash fast-path",
        SD.compare(v1_export, set(v1_paths)) == (True, [], []),
    )
    ...
    check(
        "last_diff recorded under hierarchy",
        json.load(open(mpath))["hierarchy"].get("last_diff") == a_diff,
    )
    check(
        "last_diff invisible to compare", SD.compare(export, paths_b) == (True, [], [])
    )
    SD.write_manifest(export, paths_b)
    check(
        "clean write drops last_diff",
        "last_diff" not in json.load(open(mpath))["hierarchy"],
    )
    ...
    SD.write_manifest(export, paths_b)
    check(
        "write sweeps v2-era companions",
        not any(
            os.path.exists(p)
            for p in (
                mpath + ".prev",
    ...
    check(
        "compare detects missing",
        not match and set(missing) == {"Grp|B", "Grp|C"} and not extra,
    )
    match, missing, extra = SD.compare(export, paths_b | {"Grp|D"})
    check("compare detects extra", not match and extra == ["Grp|D"])
    check(
        "compare with no manifest → match (nothing to diff)",
    ...
    match, missing, extra = SD.compare(fb_export, {"A", "A|B"})
    check(
        "deleted manifest falls back to .prev baseline",
        match and not missing and not extra,
    )
    match, missing, extra = SD.compare(fb_export, {"A"})
    check("fallback baseline still detects drift", not match and missing == ["A|B"])
    check(
        "read_manifest falls back to .prev", SD.read_manifest(fb_export) == {"A", "A|B"}
    )
    with open(
        fb_manifest, "w"
    ) as f:  # fresh file (the hidden original was removed above)
        f.write("not json{")
    match, missing, extra = SD.compare(fb_export, {"A", "A|B"})
    ...
    check(
        "find_legacy_manifest finds the versioned sidecar",
    ...
    check(
        "ensure_base_name migrates legacy to base-stem",
        migrated and os.path.basename(migrated) == ".asset.scene_data.json",
    )
    ...
    check(
        "migrate_legacy promotes the v1 name",
        promoted
        and os.path.basename(promoted) == ".oldname.scene_data.json"
        and not os.path.exists(os.path.join(tmp, ".oldname.hierarchy.json")),
    )
    check(
        "migrate_legacy carries the .prev backup",
        os.path.isfile(promoted + ".prev")
        and not os.path.exists(os.path.join(tmp, ".oldname.hierarchy.json.prev")),
    )
    ...
    check(
        "find_legacy_manifest matches v1-named versions",
    ...
    check(
        "count_descendants counts subtree",
        SD.count_descendants("Grp", {"Grp", "Grp|A", "Grp|A|Leaf", "Other"}) == 3,
    )
    ...
    check(
        "build_clean_path_set dedups",
        SD.build_clean_path_set(["A", "A", "B"]) == {"A", "B"},
    )
    ...
    check(
        "expand_to_descendants includes root + all descendants",
        expanded == {"Root", "Root|Kid", "Root|Kid|Grand"},
        str(expanded),
    )
    ...
    check(
        "format_diff_report renders the report",
        "Hierarchy Diff Report" in report
        and "Missing:  1" in report
        and "  + Grp|New" in report,
    )
    check(
        "no report file beside the export",
        not os.path.isfile(SD.diff_report_path_for(export)),
    )
