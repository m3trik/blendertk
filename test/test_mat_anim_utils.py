...
try:
    ...
    check(
        "select_by_material selects + activates",
        sel == [b]
        and b.select_set is not None
    ...
    check(
        "find_unassigned finds the material-less object",
        unassigned == [bare],
        f"{[o.name for o in unassigned]}",
    )
    check(
        "find_unassigned excludes assigned objects",
        not ({a, b} & set(btk.find_unassigned())),
    )
    bare.data.materials.append(None)  # an EMPTY slot is still "no material"
    check(
        "find_unassigned counts an empty slot as unassigned",
        btk.find_unassigned() == [bare],
        f"{[o.name for o in btk.find_unassigned()]}",
    )
    check(
        "find_unassigned scopes to the given objects",
        btk.find_unassigned(objects=[a]) == [],
        f"{btk.find_unassigned(objects=[a])}",
    )
    bpy.ops.object.empty_add(location=(9, 0, 0))
    check(
        "find_unassigned ignores objects that can't hold materials",
        btk.find_unassigned() == [bare],
        f"{[o.name for o in btk.find_unassigned()]}",
    )
    check(
        "find_unassigned is the complement of find_by_mat_id",
    ...
    check(
        "get_scene_mats inc name filter",
        only_m1 == [m1],
        f"{[m.name for m in only_m1]}",
    )
    ...
    check(
        "get_texture_paths finds the node image",
        any("diffuse.png" in p for p in paths),
        f"{paths}",
    )
    tinfo = btk.get_texture_info(materials=[m1])
    check(
        "get_texture_info reads native size",
        tinfo
        and tinfo[0]["width"] == 64
        and tinfo[0]["height"] == 32
        and tinfo[0]["mode"] == "RGBA",
        f"{tinfo}",
    )
    ...
    check(
        "get_mat_info record schema",
        rec is not None
        and rec["type"]
        and len(rec["textures"]) == 1
        and rec["textures"][0]["width"] == 64,
        f"{rec}",
    )
    check(
        "get_mat_info type = surface shader label",
        rec and "BSDF" in rec["type"],
        rec and rec["type"],
    )
    ...
    check(
        "get_mat_info exclude_unassigned drops orphan",
    ...
    check(
        "find duplicate-texture materials",
        any({m1, dup_mat} <= set(g) for g in groups),
        f"{[[m.name for m in g] for g in groups]}",
    )
    reassigned = btk.reassign_duplicate_materials(groups, delete=True)
    check(
        "reassign duplicates -> c now uses the canonical mat",
        reassigned >= 1
        and c.material_slots[0].material is m1
    ...
    check(
        "delete_unused_materials removes orphan",
        "Orphan" in removed and "Orphan" not in {m.name for m in bpy.data.materials},
        f"removed={removed}",
    )

    # fake-user material is protected
    keep = btk.create_mat("standard", name="KeepMe")
    keep.use_fake_user = True
    btk.delete_unused_materials()
    check(
        "delete_unused_materials respects fake user",
        "KeepMe" in {m.name for m in bpy.data.materials},
    )
    ...
    p_ok = os.path.join(tmp, "diffuse.png")
    make_png(p_ok)
    ...
    check(
        "get_image_records: present texture exists",
        recs[img_ok.name]["exists"] is True
        and recs[img_ok.name]["abspath"] == os.path.normpath(p_ok),
    )
    check(
        "get_image_records: missing texture flagged",
    ...
    check(
        "resolve_missing_textures repaths by basename",
        n >= 1
        and absp(img_missing)
        == os.path.normpath(os.path.join(tmp, "search", "normal.png")),
    ...
    check(
        "normalize_texture_paths copy brings externals into project",
        n >= 1
        and os.path.exists(os.path.join(proj, "diffuse.png"))
        and os.path.dirname(absp(img_ok)) == os.path.normpath(proj),
        f"n={n}",
    )
    ...
    make_png(
        os.path.join(rtdir, "rock2_AO.png")
    )  # same map type (AO), fuzzy base "rock2"
    make_png(
        os.path.join(rtdir, "rock_Normal.png")
    )  # different map type, exact base "rock"
    ...
    check(
        "resolve_missing 'texture' tier matches same map type, not the closer wrong-type name",
    ...
    for im in [i for i in bpy.data.images if i.source == "FILE"]:
        bpy.data.images.remove(im)
    csrc = os.path.join(tmp, "csrc", "tile.png")
    make_png(csrc)
    ...
    check(
        "collision guard: different-size dst not overwritten; image keeps its path",
        n == 0
        and os.path.getsize(os.path.join(cdst, "tile.png")) == size_before
    ...
    shutil.copy2(
        csrc, os.path.join(cdst2, "tile.png")
    )  # identical file already present
    n2 = btk.set_texture_directory([cimg], cdst2, mode="copy")
    check(
        "collision guard: identical dst is a safe rebind (repath, no error)",
        n2 == 1 and os.path.dirname(absp(cimg)) == os.path.normpath(cdst2),
        f"n2={n2}",
    )
    ...
    shared = os.path.join(dv_tmp, "shared_Normal.png")
    make_png(shared)
    ...
    check(
        "same texture on different surface sockets -> NOT duplicates",
    ...
    check(
        "verify=False still nets the tiling near-miss (the pre-fix candidate)",
        find_pair(_find(materials=[m_flat, m_tiled], verify=False), m_flat, m_tiled),
    )
    check(
        "same texture, different tiling/mapping -> NOT duplicates (verified)",
    ...
    next(n for n in m_set_b.node_tree.nodes if n.type == "BSDF_PRINCIPLED").inputs[
        "Metallic"
    ].default_value = 1.0
    check(
        "same texture, different unlinked shader settings -> NOT duplicates",
    ...
    check(
        "same texture, different color space -> NOT duplicates",
    ...
    pA = os.path.join(dv_tmp, "setA", "albedo.png")
    make_png(pA)
    ...
    genB.filepath_raw = pB
    genB.file_format = "PNG"
    genB.save()
    bpy.data.images.remove(genB)
    m_cnt_a = pbr_mat("DupCntA", pA)
    m_cnt_b = pbr_mat("DupCntB", pB)
    check(
        "same-basename fingerprint groups them unverified (consolidation net)",
        find_pair(_find(materials=[m_cnt_a, m_cnt_b], verify=False), m_cnt_a, m_cnt_b),
    )
    check(
        "same basename, different file content -> NOT duplicates (verified)",
    ...
    check(
        "same basename, identical content elsewhere -> verified duplicates",
    ...
    check(
        "texture inside a node group is seen (duplicates its direct twin)",
    ...
    check(
        "true duplicates (same image, same settings) are found",
    ...
    check(
        "true duplicates merge: object reassigned to keeper, dup deleted",
        n_re >= 1
        and duo.material_slots[0].material is m_true_a
    ...
    check(
        "reassign_duplicate_materials(verify=True) skips an unverified group",
    ...
    check(
        "get_shader_templates lists presets",
        "Metal" in templates and "Glass" in templates,
    )
    mat_metal = btk.create_shader_template("Metal", name="TmplMetal")
    bsdf = next(
        (n for n in mat_metal.node_tree.nodes if n.type == "BSDF_PRINCIPLED"), None
    )
    check(
        "create_shader_template builds a configured material",
        mat_metal.name == "TmplMetal"
    ...
    check(
        "apply_shader_template writes Principled inputs",
        "Roughness" in applied and abs(gb.inputs["Roughness"].default_value) < 1e-6,
        f"{applied}",
    )
    check(
        "apply_shader_template unknown template -> []",
    ...
    def st_png(mapname):
        p = os.path.join(st_tmp, f"STAsset_{mapname}.png")
        make_png(p)
        return p
    ...
    fresh = [
        os.path.join(st_tmp, "fresh", f"Brick_{m}.png")
        for m in ("BaseColor", "Roughness", "Normal")
    ]
    for f in fresh:
        make_png(f)
    ...
    check(
        "restore_material rebuilds the BSDF + link topology",
        rb is not None
        and len(rnt.links) >= 3
        and rb.inputs["Base Color"].is_linked
        and rb.inputs["Roughness"].is_linked
    ...
    base_img = next(
        (
            ...
            for n in rnt.nodes
            if n.type == "TEX_IMAGE"
            and "Brick_BaseColor" in (n.image.name if n.image else "")
        ),
        None,
    )
    check(
        "restore_material rebinds FRESH textures by map type",
        base_img is not None
        and any(
            n.type == "TEX_IMAGE"
            and n.image
            and "Brick_Normal" in n.image.name
    ...
    ptk.PresetStore("st_test", package="blendertk", user_dir=pdir).save(
        "MyShader", snap
    )
    store2 = ptk.PresetStore("st_test", package="blendertk", user_dir=pdir)
    check(
        "PresetStore save/list/rename round-trips a template",
        "MyShader" in store2.list(tier="user")
        and store2.rename("MyShader", "MyShader2")
        and "MyShader2" in store2.list(tier="user"),
    )
    from_store = btk.restore_material(
        store2.load("MyShader2"), name="STFromStore", textures=fresh
    )
    check(
        "restore_material from a PresetStore-loaded preset rebuilds the graph",
        next(
            (n for n in from_store.node_tree.nodes if n.type == "BSDF_PRINCIPLED"), None
        )
        is not None
    ...
    pm = btk.restore_material(
        {"params": {"Metallic": 1.0, "Roughness": 0.2}}, name="STParam"
    )
    pb = next((n for n in pm.node_tree.nodes if n.type == "BSDF_PRINCIPLED"), None)
    check(
        "restore_material param shorthand builds a Principled",
    ...
    check(
        "restore_material leaves image nodes unbound when no textures given",
    ...
    check(
        "ensure_image_deps empty request -> []",
        btk.ensure_image_deps(packages={}) == [],
    )
    ...
    check(
        "update_materials: no textures -> {}",
        btk.update_materials(materials=[bare]) == {},
    )

    # materials_for_textures: the "Browse…" scope — find materials referencing picked files.
    mat_b, img_b = mat_with_texture(
        "MU_Browse", os.path.join(mu_tmp, "MUBrowse_Diffuse.png")
    )
    hit = btk.materials_for_textures([os.path.join(mu_tmp, "MUBrowse_Diffuse.png")])
    check(
        "materials_for_textures finds the material referencing a picked texture",
        [m.name for m in hit] == ["MU_Browse"],
        f"{[m.name for m in hit]}",
    )
    check(
        "materials_for_textures: unreferenced path -> []",
    ...
    mat_dry, img_dry = mat_with_texture(
        "MU_Dry", os.path.join(mu_tmp, "MUDry_Diffuse.png")
    )
    before = absp(img_dry)
    res_dry = btk.update_materials(materials=[mat_dry], config={"dry_run": True})
    check(
        "update_materials dry_run: plan only, path unchanged",
    ...
    if "PIL" in btk.ensure_image_deps():
        ...
        mat_run, img_run = mat_with_texture(
            "MU_Run", os.path.join(mu_tmp, "MURun_Diffuse.png")
        )
        orig = absp(img_run)
        ...
        new = absp(img_run)
        check(
            "update_materials reprocess: image repathed to a real new file",
            res.get("MU_Run", {}).get("updated", 0) >= 1
            and new != orig
            and os.path.isfile(new),
            f"{res} -> {new}",
        )
    else:
        lines.append(
            "OK   update_materials full reprocess SKIPPED (no PIL in this Blender)"
        )
    ...
    mat_q, _img_q = mat_with_texture(
        "MU_Quiet", os.path.join(mu_tmp, "MUQuiet_Diffuse.png")
    )
    ...
    bsdf = (
        next((n for n in nt.nodes if n.type == "BSDF_PRINCIPLED"), None) if nt else None
    )
    check(
        "create_pbr_material builds a material with a Principled BSDF",
    ...
    def img_cs(
        maptype,
    ):  # colorspace of the loaded image whose filename carries maptype
        node = next(
            (n for n in nt.nodes if n.type == "TEX_IMAGE" and maptype in n.image.name),
            None,
        )
        return node.image.colorspace_settings.name if node else None

    check(
        "metallic + roughness wired from images",
        getattr(src_node("Metallic"), "type", "") == "TEX_IMAGE"
        and getattr(src_node("Roughness"), "type", "") == "TEX_IMAGE",
    )
    check(
        "normal wired through a Normal Map node",
        getattr(src_node("Normal"), "type", "") == "NORMAL_MAP",
    )
    check(
        "emission + alpha wired",
        src_node("Emission Color") is not None and src_node("Alpha") is not None,
    )
    check(
        "AO multiplied into Base Color (MixRGB)",
        getattr(src_node("Base Color"), "type", "") == "MIX_RGB",
    )
    check(
        "color space: base sRGB, data Non-Color",
        img_cs("BaseColor") == "sRGB"
        and img_cs("Metallic") == "Non-Color"
    ...
    rough_src = (
        gb.inputs["Roughness"].links[0].from_node
        if gb.inputs["Roughness"].is_linked
        else None
    )
    check(
        "glossiness inverted into roughness", getattr(rough_src, "type", "") == "INVERT"
    )

    # DirectX normal -> green-flip graph (SeparateColor present)
    dmat = btk.create_pbr_material([gs_png("Normal_DirectX")], name="GSDX")
    check(
        "DirectX normal builds a green-flip (SeparateColor)",
    ...
    check(
        "ORM split feeds Metallic + Roughness",
        ob.inputs["Metallic"].is_linked
        and ob.inputs["Roughness"].is_linked
    ...
    check(
        "AlbedoTransparency filename classifies",
    ...
    base_src = (
        ab.inputs["Base Color"].links[0].from_node
        if ab.inputs["Base Color"].is_linked
        else None
    )
    alpha_src = (
        ab.inputs["Alpha"].links[0].from_node if ab.inputs["Alpha"].is_linked else None
    )
    ...
    check(
        "AlbedoTransparency: one TEX_IMAGE drives both Base Color and Alpha",
        getattr(base_src, "type", "") == "TEX_IMAGE"
        and getattr(alpha_src, "type", "") == "TEX_IMAGE"
        and base_src.name == alpha_src.name
        and n_at_imgs == 1
    ...
    check(
        "MetallicSmoothness filename classifies",
    ...
    check(
        "MetallicSmoothness feeds Metallic (image) + Roughness (invert)",
    ...
    check(
        "MaskMap filename classifies as MSAO",
    ...
    check(
        "MSAO feeds Metallic (SeparateColor) + Roughness (invert) + AO (MixRGB into Base)",
    ...
    check(
        "rival packings collapse to one: ORM supersedes MSAO + MetallicSmoothness",
    ...
    check(
        "rival packings: only the surviving ORM image is loaded",
        n_imgs == 1,
        f"images={n_imgs}",
    )
    ...
    check(
        "the surviving packed map applies AO exactly once (no stacked MULTIPLY)",
        n_mix == 1,
        f"mix_nodes={n_mix}",
    )
    ...
    def gs_set_png(setname, mapname):
        p = os.path.join(gs_tmp, f"{setname}_{mapname}.png")
        make_png(p)
        return p

    batch_files = [
        gs_set_png("brick", "BaseColor"),
        gs_set_png("brick", "Normal"),
        gs_set_png("wood", "BaseColor"),
        gs_set_png("wood", "Roughness"),
    ]
    batch = btk.create_pbr_materials(batch_files)
    check(
        "create_pbr_materials groups by set -> 2 materials",
        set(batch) == {"brick", "wood"} and all(m is not None for m in batch.values()),
        f"{list(batch)}",
    )
    merged = btk.create_pbr_materials(batch_files, name="Merged")
    check(
        "create_pbr_materials explicit name -> single material",
    ...
    check(
        "create_pbr_materials prefix batches + prepends per set",
        set(pref) == {"Mat_brick", "Mat_wood"}
        and all(m is not None for m in pref.values()),
        f"{list(pref)}",
    )
    check(
        "create_pbr_materials nothing on disk -> {}",
        btk.create_pbr_materials([os.path.join(gs_tmp, "ghost.png")]) == {},
    )

    check(
        "create_pbr_material no classifiable textures -> None",
    ...
    noise = os.path.join(gs_tmp, "random_noise.png")
    make_png(noise)
    plan = btk.MatUtils.resolve_pbr_plan([gs_png("BaseColor"), gs_png("Normal"), noise])
    check(
        "resolve_pbr_plan returns the documented shape",
        set(plan)
        == {"by_type", "dropped", "extracted", "unknown", "unhandled", "wired"},
        f"{sorted(plan)}",
    )
    check(
        "resolve_pbr_plan classifies known maps + collects unclassified ones",
    ...
    check(
        "resolve_pbr_plan flags a classified-but-unwirable map as unhandled",
        set(plan_un["unhandled"]) == {"Thickness"},
        f"{plan_un['unhandled']}",
    )
    ...
    check(
        "MRAO supersedes loose Metallic/Roughness (shared precedence rules)",
        set(mr_plan["dropped"]) == {"Metallic", "Roughness"}
        and "MRAO" in mr_plan["by_type"],
    ...
    check(
        "MRAO is WIRED, so superseding the loose maps loses nothing",
        mrb.inputs["Metallic"].is_linked
        and mrb.inputs["Roughness"].is_linked
    ...
    check(
        "create_pbr_material reports the map types it actually wired",
        "wired" in sh_plan
        and {"Base_Color", "Normal", "Roughness"} <= sh_plan["wired"],
        f"wired={sorted(sh_plan.get('wired', ()))}",
    )
    check(
        "shadowed maps (Height under Normal, Glossiness under Roughness) are NOT 'wired'",
    ...
    ogl = btk.create_pbr_material(
        [gs_png("Normal_OpenGL")], name="GSNrmOGL", normal_direction="DirectX"
    )
    check(
        "explicit Normal_OpenGL is NOT flipped even when DirectX is requested",
        _normal_chain(ogl) == "TEX_IMAGE",
        f"{_normal_chain(ogl)}",
    )
    dx = btk.create_pbr_material(
        [gs_png("Normal_DirectX")], name="GSNrmDX", normal_direction="OpenGL"
    )
    check(
        "explicit Normal_DirectX IS flipped even when OpenGL is requested",
        _normal_chain(dx) == "COMBINE_COLOR",
        f"{_normal_chain(dx)}",
    )
    amb = btk.create_pbr_material(
        [gs_png("Normal")], name="GSNrmAmb", normal_direction="DirectX"
    )
    check(
        "ambiguous Normal follows the requested direction",
        _normal_chain(amb) == "COMBINE_COLOR",
        f"{_normal_chain(amb)}",
    )
    ...
    both = btk.create_pbr_material(
        [gs_png("Normal"), gs_png("Normal_OpenGL")],
        name="GSNrmBoth",
        normal_direction="DirectX",
    )
    check(
        "explicit Normal_OpenGL outranks an ambiguous Normal in the same set",
        _normal_chain(both) == "TEX_IMAGE",
        f"{_normal_chain(both)}",
    )
    both_dx = btk.create_pbr_material(
        [gs_png("Normal"), gs_png("Normal_DirectX")],
        name="GSNrmBothDX",
        normal_direction="OpenGL",
    )
    check(
        "explicit Normal_DirectX outranks an ambiguous Normal in the same set",
        _normal_chain(both_dx) == "COMBINE_COLOR",
        f"{_normal_chain(both_dx)}",
    )
    ...
    check(
        "resolve_pbr_plan config=None -> packed ORM supersedes its loose components",
        set(plan_pk["by_type"]) == {"ORM"}
        and set(plan_pk["dropped"]) == {"Metallic", "Roughness"},
    ...
    gmat = gs.create_network(
        gs_files, name="GSNet", progress_callback=lambda p, m: prog.append(p)
    )
    check(
        "GameShader.create_network builds one named material",
        getattr(gmat, "name", None) == "GSNet",
    )
    check(
        "GameShader.create_network drives progress_callback to 100",
        prog and prog[-1] == 100,
        f"{prog}",
    )
    check(
        "GameShader.create_network mirrors mtk.GameShader's entry point",
    ...
    gs_batch = gs.create_network(
        [
            gs_set_png("stone", "BaseColor"),
            gs_set_png("stone", "Normal"),
            gs_set_png("metal", "BaseColor"),
        ]
    )
    check(
        "GameShader.create_network batches unnamed input into one material per set",
        isinstance(gs_batch, list)
        and len(gs_batch) == 2
    ...
    check(
        "GameShader.create_network no textures -> None", gs.create_network([]) is None
    )
    ...
    check(
        "GameShader.create_network applies a prefix to the resolved name",
        getattr(gs_pre, "name", "") == "MAT_slate",
        f"{getattr(gs_pre, 'name', gs_pre)}",
    )
    ...
    if "PIL" in btk.ensure_image_deps():
        ...
        loaded = (
            {n.image.name for n in packed.node_tree.nodes if n.type == "TEX_IMAGE"}
            if packed
            else set()
        )
        check(
            "GameShader.create_network workflow profile generates + wires a packed ORM",
            any("ORM" in n for n in loaded),
            f"{sorted(loaded)}",
        )
        check(
            "GameShader.create_network packed profile drops the loose components it packed",
            not any(
                ("Roughness" in n or "Metallic" in n or "Occlusion" in n)
                for n in loaded
            ),
            f"{sorted(loaded)}",
        )
        ...
        lines.append(
            "OK   GameShader workflow-profile packing SKIPPED (no PIL in this Blender)"
        )
    ...
    htypes = {
        k.handle_left_type for fc in btk.get_fcurves(a) for k in fc.keyframe_points
    }
    ...
    check(
        "scale_keys about pivot=0 doubles times",
        key_times(a) == [t * 2 for t in t0],
        f"{t0}->{key_times(a)}",
    )
    ...
    check(
        "fit_playback_range",
        rng == (sc.frame_start, sc.frame_end) and sc.frame_start == 6,
        f"{rng}",
    )
    ...
    check(
        "move_keys_to_frame per-action aligns first keys",
    ...
    check(
        "move_keys_to_frame defaults to current frame",
        key_times(a)[0] == 60.0,
        f"{key_times(a)}",
    )
    sc.frame_set(sc0)
    ...
    check(
        "adjust_key_spacing shifts only keys at/after the frame",
        moved == 1 and key_times(a) == [60.0, 85.0],
        f"{key_times(a)}",
    )
    check(
        "adjust_key_spacing no keys after frame -> 0",
    ...
    check(
        "align_selected_keyframes moves only the selected key",
        moved == 1 and key_times(a) == [60.0, 70.0],
        f"{key_times(a)}",
    )
    for k in [k for fc in btk.get_fcurves(a) for k in fc.keyframe_points]:
        k.select_control_point = False
    ...
    check(
        "add_intermediate_keys fills the span",
        added == 9 and len(key_times(a)) == 11,
        f"added={added} n={len(key_times(a))}",
    )
    removed = btk.remove_intermediate_keys(a)
    check(
        "remove_intermediate_keys keeps endpoints",
        removed == 9 and key_times(a) == [60.0, 70.0],
        f"{key_times(a)}",
    )
    ...
    check(
        "select_keys before",
        n == 1 and selected_times() == [60.0],
        f"n={n} sel={selected_times()}",
    )
    n = btk.select_keys(a, time="after")
    check(
        "select_keys after",
        n == 1 and selected_times() == [70.0],
        f"n={n} sel={selected_times()}",
    )
    ...
    check(
        "select_keys before|current",
        n == 1 and selected_times() == [60.0],
        f"n={n} sel={selected_times()}",
    )
    n = btk.select_keys(a, time="after|current")
    check(
        "select_keys after|current",
        n == 2 and selected_times() == [60.0, 70.0],
        f"n={n} sel={selected_times()}",
    )
    try:
    ...
    vis_curves = [
        fc
        for fc in btk.get_fcurves(b)
        if fc.data_path in ("hide_viewport", "hide_render")
    ]
    check(
        "set_visibility_keys keys both hide props",
        keyed == [b]
        and len(vis_curves) == 2
    ...
    check(
        "copy/paste keys independent copy",
    ...
    for f, v in ((1, 0.0), (10, 0.0), (20, 0.0), (30, 5.0)):  # flat hold then ramp on z
        c.location.z = v
        c.keyframe_insert("location", index=2, frame=f)
    stats = {}
    btk.optimize_keys(c, stats=stats)
    x_curves = [
        fc
        for fc in btk.get_fcurves(c)
        if fc.data_path == "location" and fc.array_index == 0
    ]
    check(
        "optimize_keys removes static curve (value preserved)",
    ...
    check(
        "optimize_keys removes interior flat key (keeps boundaries)",
        ztimes == [1.0, 20.0, 30.0],
        f"{ztimes}",
    )
    check(
        "optimize_keys stats reduced",
    ...
    for f in range(0, 201):  # 3 sine waves, a hold, a ramp -- per-frame bake
        ...
        baked[f] = v
    # A stepped visibility track must survive the flat pass, not the extremes.
    ...
    check(
        "extremes leaves the stepped curve to the flat pass",
    ...
    check(
        "tie_keyframes adds range bookends",
        changed == 2 and times == [1.0, 5.0, 15.0, 30.0],
        f"{times}",
    )
    changed2 = btk.tie_keyframes(d, untie=True)
    times2 = sorted(k.co.x for fc in btk.get_fcurves(d) for k in fc.keyframe_points)
    check(
        "tie_keyframes untie removes only the bookends",
        changed2 == 2 and times2 == [5.0, 15.0],
        f"{times2}",
    )
    ...
    target.name = "Target"
    for f in (1, 20):
        target.location.x = float(f) * 2
        target.keyframe_insert("location", index=0, frame=f)
    bpy.ops.mesh.primitive_cube_add(location=(0, 5, 0))
    follower = bpy.context.active_object
    follower.name = "Follower"
    con = follower.constraints.new("COPY_LOCATION")
    con.target = target
    try:
        ...
        check(
            "bake_keys bakes the constrained follower + clears the constraint",
        ...
        check(
            "bake_keys (context-limited headless)", "context" in str(e).lower(), repr(e)
        )
    ...
    check(
        "bake_blend_shapes baked the driven mesh",
        baked == [shp],
        f"{[o.name for o in baked]}",
    )
    ...
    check(
        "bake_blend_shapes captured driver values (f5~0.5, f10~1.0)",
        abs(v5 - 0.5) < 0.05 and abs(v10 - 1.0) < 0.05,
        f"v5={v5:.3f} v10={v10:.3f}",
    )
    ...
    check(
        "get_recent_autosave (path, stamp) pairs",
        isinstance(autosaves, list) and all(len(t) == 2 for t in autosaves),
    )
    ...
    check(
        "get_scene_info counts objects",
        info["objects"] == 4 and info["meshes"] == 2,
        f"{info}",
    )
    check(
        "get_scene_info mesh stats (2 cubes)",
        info["vertices"] == 16 and info["faces"] == 12 and info["triangles"] == 24,
        f"{info}",
    )
    check(
        "get_scene_info lights + cameras", info["lights"] == 1 and info["cameras"] == 1
    )
    check("get_scene_info no-material meshes", info["meshesWithoutMaterial"] == 2)
    one = btk.get_scene_info([cube])
    check(
        "get_scene_info scoped to one object",
        one["objects"] == 1 and one["meshes"] == 1,
    )
    html = btk.format_scene_info_html(info)
    check(
        "format_scene_info_html builds report",
    ...
    keep_me = bpy.data.meshes.new("KeepMesh")
    keep_me.use_fake_user = True  # survives
    removed = btk.cleanup_scene(quiet=True)
    mesh_names = [m.name for m in bpy.data.meshes]
    check(
        "cleanup_scene purges orphans, keeps fake-user + in-use",
        "OrphanMesh" not in mesh_names
        and "KeepMesh" in mesh_names
        and big.data.name in mesh_names
        and removed.get("meshes", 0) >= 1,
        f"{removed}",
    )
    ...
    check(
        "fix_color_spaces sets data maps -> Non-Color",
        nrm.colorspace_settings.name == "Non-Color"
        and rgh.colorspace_settings.name == "Non-Color",
        f"{changed}",
    )
    check(
        "fix_color_spaces leaves correct color map",
        alb.colorspace_settings.name == "sRGB",
    )
    check(
        "fix_color_spaces reports only genuine changes",
        set(changed) == {nrm.name, rgh.name},
        f"{set(changed)}",
    )
    check(
        "fix_color_spaces ignores unrecognized map",
    ...
    check(
        "fix_color_spaces dry_run reports without writing",
    ...
    check(
        "fix_color_spaces corrects color map -> sRGB",
        alb2.colorspace_settings.name == "sRGB",
    )
    ...
    check(
        "fix_color_spaces scan-all path fixes new data map",
        nrm3.colorspace_settings.name == "Non-Color" and nrm3.name in allchanged,
        f"{allchanged}",
    )
    ...
    btk.configure_render_output(
        sc, file_format="FFMPEG", container="MPEG4", codec="H264", quality=90
    )
    check(
        "configure_render_output FFMPEG MP4/H264 + quality->CRF",
        sc.render.image_settings.file_format == "FFMPEG"
        and sc.render.ffmpeg.format == "MPEG4"
        and sc.render.ffmpeg.codec == "H264"
    ...
    check(
        "configure_render_output JPEG + quality",
        sc.render.image_settings.file_format == "JPEG"
        and sc.render.image_settings.quality == 50,
    )
