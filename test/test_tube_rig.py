...
try:
    ...
    # ---- surface: classes exposed, register is a classmethod (no flat register_strategy)
    check(
        "tube_rig classes exposed; register is class-only (no flat register_strategy)",
        btk.TubeRig is TubeRig
        and btk.TubeStrategy is TubeStrategy
        and callable(TubeStrategy.register)
        and not hasattr(btk, "register_strategy"),
    )
    ...
    def tube(name="Tube", radius=0.5, depth=8.0, segs=12, rings=10):
        ...
        bmesh.ops.subdivide_edges(
            bm, edges=bm.edges[:], cuts=rings, use_grid_fill=False
        )
        bm.to_mesh(me)
        bm.free()
    ...
    def eval_bounds(obj):
        ...
        try:
            ...
            mn = Vector(
                (min(p.x for p in ws), min(p.y for p in ws), min(p.z for p in ws))
            )
            mx = Vector(
                (max(p.x for p in ws), max(p.y for p in ws), max(p.z for p in ws))
            )
            return mn, mx
        finally:
            ev.to_mesh_clear()
    ...
    check(
        "registry has the 3 built-in strategies",
        set(TUBE_STRATEGIES) == {"spline", "anchor", "fk"},
        f"{sorted(TUBE_STRATEGIES)}",
    )
    spec = SplineIKStrategy()
    check(
        "strategy declares Qt-free option dicts (AttributeSpec kwargs)",
        all(isinstance(o, dict) and "key" in o and "kind" in o for o in spec.options),
    )
    check(
        "defaults derive from the option dicts",
        spec.defaults()["num_joints"] == 12
        and spec.defaults()["enable_stretch"] is True,
    )
    check(
        "resolve overrides defaults",
        spec.resolve({"num_joints": 20})["num_joints"] == 20
    ...
    check(
        "spline: armature + bones built",
        bundle.armature.type == "ARMATURE" and len(bundle.bones) == 11,
        f"{len(bundle.bones)} bones",
    )
    check(
        "spline: armature + curve + controls grouped under the rig root",
        bundle.armature.parent is bundle.root
        and bundle.curve.parent is bundle.root
        and all(c.parent is bundle.root for c in bundle.controls),
    )
    check(
        "spline: driver curve + N controls",
        bundle.curve is not None and len(bundle.controls) == 3,
        f"{len(bundle.controls)} controls",
    )
    check(
        "spline: mesh bound to the armature",
        any(m.type == "ARMATURE" and m.object is bundle.armature for m in t.modifiers),
    )
    check(
        "spline: spline IK constraint on the tip bone",
        any(
            c.type == "SPLINE_IK"
            for c in bundle.armature.pose.bones[bundle.bones[-1]].constraints
        ),
    )
    ...
    _, mx1 = eval_bounds(t)
    check(
        "spline: moving a control bends the mesh (x-extent grows)",
        mx1.x > mx0.x + 1.0,
    ...
    check(
        "spline squash off: cross-section stays ~constant on stretch (NONE)",
        abs(r_none - r0) < 0.02,
        f"radius {r0:.3f} -> {r_none:.3f}",
    )
    check(
        "spline squash+volume: stretch shrinks the cross-section (VOLUME_PRESERVE)",
        r_vol < r0 - 0.02,
        f"radius {r0:.3f} -> {r_vol:.3f}",
    )
    check(
        "spline squash w/o volume thins MORE (INVERSE_PRESERVE < VOLUME_PRESERVE)",
        r_inv < r_vol - 0.01,
        f"volume={r_vol:.3f} inverse={r_inv:.3f}",
    )
    ...
    _, mx1 = eval_bounds(t)
    check(
        "spline auto_bend: compressing the ends bulges the middle (+Y)",
        mx1.y > mx0.y + 0.3,
    ...
    _, mx1 = eval_bounds(t)
    check(
        "spline auto_bend off: compression does not bulge the middle",
        mx1.y < mx0.y + 0.1,
        f"max-y {mx0.y:.2f} -> {mx1.y:.2f}",
    )
    ...
    twist_bone = next(
        (bn.name for bn in b.armature.data.bones if "_twist_ctrl" in bn.name), None
    )
    ...
    check(
        "twist: _ordered_chain returns the deform chain, not the isolated control bone",
        len(ordered) == 11 and twist_bone not in ordered,
        f"chain len={len(ordered)}",
    )
    ...
    check(
        "twist: rolling the control twists the tip cross-section",
        tip_d > 45,
        f"tip rotated {tip_d:.0f} deg for a 90 deg roll",
    )
    check(
        "twist: twist is progressive (start rotates far less than the tip)",
        start_d < tip_d - 30,
    ...
    check(
        "twist off: no roll-control bone built (the toggle gates it)",
    ...
    sb = rig.constrain_end_with_falloff(
        armobj, bones, a_start, t, falloff=3.0, bone_index=0
    )
    eb = rig.constrain_end_with_falloff(
        armobj, bones, a_end, t, falloff=3.0, bone_index=-1
    )
    check(
        "b004: anchor bones grafted for both ends",
        sb in armobj.data.bones and eb in armobj.data.bones,
        f"{sb}, {eb}",
    )
    check(
        "b004: falloff vertex groups created on the mesh",
    ...
    check(
        "b004: moving an anchor drags the NEAR-end skin (falloff blend)",
        end_shift > 1.0,
        f"end vertex shift={end_shift:.2f}",
    )
    check(
        "b004: the MIDDLE barely moves (falloff decays to zero by the radius)",
        mid_shift < 0.5,
        f"mid vertex shift={mid_shift:.2f}",
    )
    check(
        "b004: near-end follows the anchor MORE than the middle (monotonic falloff)",
        end_shift > mid_shift + 1.0,
    ...
    RigUtils.apply_falloff_weights(
        t2, "rd_test", vpos + Vector((0, 0, 1.0)), 4.0
    )  # d=1, r=4 -> w=0.75
    ...
    check(
        "b004: apply_falloff sets target weight = 1 - d/r (linear)",
        abs(w_target - 0.75) < 0.02,
        f"w={w_target:.3f} (expected 0.75)",
    )
    check(
        "b004: apply_falloff scales existing influences by (1-w) (redistribution, not addition)",
        abs(w_others - before * 0.25) < 0.02,
        f"others {before:.3f} -> {w_others:.3f} (want {before * 0.25:.3f})",
    )
    ...
    check(
        "b004: apply_falloff leaves non-deform groups untouched (pin keeps 0.8)",
        abs(gw2.get(pin.index, 0.0) - 0.8) < 1e-6,
        f"pin={gw2.get(pin.index)}",
    )
    ...
    b4 = TubeRig(t4, rig_name="asp").build(
        "spline", num_joints=8, num_controls=3, radius=0.6
    )
    bpy.context.view_layer.update()
    ...
    r4.constrain_end_with_falloff(
        b4.armature, b4.bones, a_end2, t4, falloff=3.0, bone_index=-1
    )
    end_ctrl4, start_ctrl4 = b4.controls[-1], b4.controls[0]
    check(
        "b004 spline: end control auto-resolved + CHILD_OF-bound to the anchor",
        any(c.type == "CHILD_OF" and c.target is a_end2 for c in end_ctrl4.constraints),
        f"{[c.type for c in end_ctrl4.constraints]}",
    )
    check(
        "b004 spline: start control NOT bound (only the constrained end routes)",
    ...
    check(
        "b004 spline: moving the anchor carries the tube end with it",
        (p1 - p0).length > 2.0,
        f"shift={(p1 - p0).length:.2f}",
    )
    ...
    check(
        "anchor: 1 bone + 2 controls",
        len(bundle.bones) == 1 and len(bundle.controls) == 2,
        f"bones={len(bundle.bones)} controls={len(bundle.controls)}",
    )
    check(
        "anchor: stretch constraint on the bone",
        any(
            c.type == "STRETCH_TO"
            for c in bundle.armature.pose.bones[bundle.bones[0]].constraints
        ),
    )
    ...
    check(
        "fk: bones are the controls",
        len(bundle.bones) == 7 and bundle.controls == list(bundle.bones),
        f"{len(bundle.bones)} bones",
    )
    check(
        "fk: each bone has a custom shape",
        all(
            bundle.armature.pose.bones[b].custom_shape is not None for b in bundle.bones
        ),
    )
    _fkshape = bundle.armature.pose.bones[bundle.bones[0]].custom_shape
    check(
        "fk: custom-shape source parented under root (no orphan)",
        _fkshape.parent is bundle.root,
    )
    ...
    check(
        "fk: rotating the root bone swings the tail (mesh follows)",
        delta > 1.0,
