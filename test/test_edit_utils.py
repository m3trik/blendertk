...
try:
    ...
    check(
        "decimate applied (no modifier left)",
        len(o.modifiers) == 0,
        f"mods={len(o.modifiers)}",
    )
    ...
    check(
        "decimate apply=False keeps modifier",
        any(m.type == "DECIMATE" for m in o.modifiers),
    )
    ...
    check(
        "set_subdivision viewport/render levels",
        sm and sm.levels == 2 and sm.render_levels == 3,
        f"v={sm.levels if sm else '?'} r={sm.render_levels if sm else '?'}",
    )
    check(
        "set_subdivision keeps base mesh (live, 6 faces)",
        faces(o) == 6,
        f"n={faces(o)}",
    )
    ...
    check(
        "set_subdivision reuses modifier",
        len(subsurfs) == 1 and subsurfs[0].levels == 1,
    ...
    check(
        "set_shading flat -> faces not smooth",
        all(not p.use_smooth for p in o.data.polygons),
    )
    btk.set_shading(o, smooth=True)
    check(
        "set_shading smooth -> faces smooth", all(p.use_smooth for p in o.data.polygons)
    )
    ...
    check(
        "average_normals -> all faces smooth",
        all(p.use_smooth for p in o.data.polygons),
    )
    check(
        "average_normals -> all edges soft (not sharp)",
    ...
    check(
        "average_normals by_uv_shell -> some UV-seam edges sharp",
        sharp > 0,
        f"sharp={sharp}",
    )
    check(
        "average_normals by_uv_shell -> not ALL edges sharp (interior stays soft)",
        sharp < len(o.data.edges),
        f"sharp={sharp}/{len(o.data.edges)}",
    )
    check(
        "average_normals by_uv_shell -> faces still smooth",
        all(p.use_smooth for p in o.data.polygons),
    )
    ...
    check(
        "set_edge_hardness upper=180 -> 0 sharp (above-threshold softened)",
        sum(1 for e in o.data.edges if e.use_edge_sharp) == 0,
        f"sharp={sum(1 for e in o.data.edges if e.use_edge_sharp)}",
    )
    ...
    check(
        "set_edge_hardness upper=None leaves above-threshold edges as-is (still 12 sharp)",
        sum(1 for e in o.data.edges if e.use_edge_sharp) == 12,
        f"sharp={sum(1 for e in o.data.edges if e.use_edge_sharp)}",
    )
    ...
    check(
        "select_edges_by_angle range covering 90 -> all 12 edges",
        n_in == 12,
        f"n={n_in}",
    )
    ...
    check(
        "flip_normals reverses normal",
        (n_before + n_after).length < 1e-4,
    ...
    p = o.data.polygons[0]
    check(
        "recalculate inside -> normal points inward",
        p.normal.dot(p.center) < 0,
        f"dot={p.normal.dot(p.center):.2f}",
    )
    btk.recalculate_normals(o, inside=False)
    p = o.data.polygons[0]
    check(
        "recalculate outside -> normal points outward",
        p.normal.dot(p.center) > 0,
        f"dot={p.normal.dot(p.center):.2f}",
    )
    ...
    p = o.data.polygons[0]
    check(
        "clean_geometry recalc -> outward normals",
        p.normal.dot(p.center) > 0,
        f"dot={p.normal.dot(p.center):.2f}",
    )
    ...
    me = bpy.data.meshes.new("Degen")
    bm.to_mesh(me)
    bm.free()
    o = bpy.data.objects.new("Degen", me)
    bpy.context.collection.objects.link(o)
    e0 = len(o.data.edges)
    btk.clean_geometry(o, merge=False, delete_loose=False, degenerate=True)
    check(
        "clean_geometry degenerate works with merge off",
        len(o.data.edges) < e0,
    ...
    me = bpy.data.meshes.new("NoMerge")
    bm.to_mesh(me)
    bm.free()
    o = bpy.data.objects.new("NoMerge", me)
    bpy.context.collection.objects.link(o)
    v0 = len(o.data.vertices)
    btk.clean_geometry(o, merge=False, delete_loose=False, degenerate=False)
    check(
        "clean_geometry merge=False keeps doubles",
        len(o.data.vertices) == v0,
    ...
    check(
        "_object_mode restores caller's active",
        bpy.context.view_layer.objects.active is a,
        f"active={getattr(bpy.context.view_layer.objects.active, 'name', None)}",
    )
    check(
        "_object_mode restores caller's EDIT mode on A",
        a.mode == "EDIT",
        f"mode={a.mode}",
    )
    bpy.ops.object.mode_set(mode="OBJECT")
    ...
    check(
        "crease_edges object-mode -> all edges 1.0",
        cl is not None and all(abs(d.value - 1.0) < 1e-4 for d in cl.data),
        "via crease_edge attr",
    )
    ...
    check(
        "crease_edges edit-mode -> only 3 selected edges 0.5",
        n_half == 3,
        f"n_0.5={n_half}",
    )
    ...
    check(
        "triangulate from EDIT restores EDIT mode",
        restored == "EDIT",
        f"mode={restored}",
    )
    ...
    check(
        "boolean_op returns base, modifier applied",
        out is base and len(base.modifiers) == 0,
    )
    check(
        "boolean_op changed geometry",
        len(base.data.vertices) != 8,
    ...
    check(
        "crease_edges angle=0 -> all edges hard",
        all(e.use_edge_sharp for e in o.data.edges),
        f"sharp={sum(1 for e in o.data.edges if e.use_edge_sharp)}",
    )
    cl = o.data.attributes.get("crease_edge")
    check(
        "crease_edges angle=0 still creases",
        cl is not None and all(abs(d.value - 1.0) < 1e-4 for d in cl.data),
    )
    ...
    check(
        "crease_edges angle=45 (< 90deg) -> edges hard",
        all(e.use_edge_sharp for e in o.data.edges),
    )
    btk.crease_edges(o, amount=10.0, angle=135)
    check(
        "crease_edges angle=135 (> 90deg) -> edges soft",
        not any(e.use_edge_sharp for e in o.data.edges),
    )
    ...
    check(
        "crease_edges angle=None leaves softness untouched",
        all(e.use_edge_sharp for e in o.data.edges),
    )
    ...
    check(
        "dissolve_coplanar delimit=SHARP keeps a split (more than 1 face)",
        len(o.data.polygons) > 1,
        f"faces={len(o.data.polygons)} (was {n_before})",
    )
    ...
    check(
        "has_custom_split_normals: no meshes -> False",
        btk.has_custom_split_normals([]) is False,
    )
    check(
        "has_custom_split_normals: fresh meshes -> False",
        btk.has_custom_split_normals([a, b]) is False,
    )

    check(
        "add_custom_split_normals locks both", btk.add_custom_split_normals([a, b]) == 2
    )
    check(
        "add_custom_split_normals -> custom_split data",
        a.data.has_custom_normals and b.data.has_custom_normals,
    )
    check(
        "has_custom_split_normals: both locked -> True",
        btk.has_custom_split_normals([a, b]) is True,
    )
    check(
        "add_custom_split_normals: already locked -> 0",
        btk.add_custom_split_normals([a, b]) == 0,
    )

    check(
        "clear_custom_split_normals unlocks both",
        btk.clear_custom_split_normals([a, b]) == 2,
    )
    check(
        "clear_custom_split_normals -> no custom_split data",
        not a.data.has_custom_normals and not b.data.has_custom_normals,
    )
    check(
        "clear_custom_split_normals: nothing locked -> 0",
        btk.clear_custom_split_normals([a, b]) == 0,
    )

    # Mixed selection reads unlocked (Maya's all() lock-state semantics), so a toggle locks it whole.
    btk.add_custom_split_normals([a])
    check(
        "has_custom_split_normals: mixed -> False",
        btk.has_custom_split_normals([a, b]) is False,
    )
    check(
        "add_custom_split_normals: mixed -> only the unlocked one counts",
    ...
    bpy.ops.object.mode_set(mode="EDIT")
    check(
        "has_custom_split_normals from Edit Mode",
        btk.has_custom_split_normals([a, b]) is True,
    )
    check(
        "clear_custom_split_normals from Edit Mode",
        btk.clear_custom_split_normals([a, b]) == 2,
    )
    ...
    check(
        "has_custom_split_normals: non-mesh -> False",
        btk.has_custom_split_normals([e]) is False,
    )
    check(
        "add_custom_split_normals: non-mesh -> 0",
        btk.add_custom_split_normals([e]) == 0,
    )
    ...
    bpy.ops.mesh.primitive_cube_add()
    o = bpy.context.active_object
    c = btk.Diagnostics.find_problem_geometry(
        o,
        ngons=True,
        concave=True,
        nonplanar=True,
    ...
    check(
        "find_problem clean cube -> all zero",
        all(
            c[k] == 0
            for k in (
                "ngons",
    ...
    o = bpy.context.active_object
    c = btk.Diagnostics.find_problem_geometry(o, ngons=True, select=True)
    ...
    bpy.context.view_layer.objects.active = o
    bpy.ops.object.mode_set(mode="EDIT")
    btk.Diagnostics.find_problem_geometry(o, ngons=True, select=True)
    check(
        "find_problem from EDIT restores EDIT mode", o.mode == "EDIT", f"mode={o.mode}"
    )
    bpy.ops.object.mode_set(mode="OBJECT")  # flush -> select flags must still be there
    check(
        "find_problem from EDIT keeps the ngon selected",
        any(p.select for p in o.data.polygons),
    )
    ...
    bm.faces.new(vs)
    me = bpy.data.meshes.new("NP")
    bm.to_mesh(me)
    bm.free()
    o = bpy.data.objects.new("NP", me)
    bpy.context.collection.objects.link(o)
    c = btk.Diagnostics.find_problem_geometry(o, nonplanar=True, select=False)
    check(
        "find_problem detects non-planar quad",
        c["nonplanar"] == 1,
        f"nonplanar={c['nonplanar']}",
    )
    reset()
    bpy.ops.mesh.primitive_plane_add()
    o = bpy.context.active_object
    c = btk.Diagnostics.find_problem_geometry(o, nonplanar=True, select=False)
    check(
        "find_problem flat plane -> planar",
        c["nonplanar"] == 0,
        f"nonplanar={c['nonplanar']}",
    )
    ...
    me = bpy.data.meshes.new("Open")
    bm.to_mesh(me)
    bm.free()
    o = bpy.data.objects.new("Open", me)
    bpy.context.collection.objects.link(o)
    c = btk.Diagnostics.find_problem_geometry(o, nonmanifold=True, select=False)
    check(
        "find_problem detects 4 nonmanifold (boundary) edges",
        c["nonmanifold"] == 4,
    ...
    bm.faces.new(vs)
    me = bpy.data.meshes.new("Dart")
    bm.to_mesh(me)
    bm.free()
    o = bpy.data.objects.new("Dart", me)
    bpy.context.collection.objects.link(o)
    c = btk.Diagnostics.find_problem_geometry(o, concave=True, select=False)
    check(
        "find_problem detects concave dart quad",
        c["concave"] == 1,
        f"concave={c['concave']}",
    )
    reset()
    ...
    c = btk.Diagnostics.find_problem_geometry(o, concave=True, select=False)
    check(
        "find_problem square plane -> not concave",
        c["concave"] == 0,
        f"concave={c['concave']}",
    )
    ...
    v = [
        ...
        for co in (
            ...
            (0, 1, 0),  # base quad 0-3
            (1, 0, 1),
            (1, 1, 1),  # +Z wall pts 4-5
            (1, 0, -1),
            (1, 1, -1),  # -Z wall pts 6-7
        )
    ]
    ...
    bm.faces.new(
        (v[1], v[6], v[7], v[2])
    )  # -Z, shares edge v1-v2  -> edge v1-v2 has 3 faces
    ...
    c = btk.Diagnostics.find_problem_geometry(o, nonmanifold=True, select=False)
    check(
        "find_problem detects nonmanifold shared edge",
        c["nonmanifold"] >= 1,
        f"nonmanifold={c['nonmanifold']}",
    )
    ...
    bm = bmesh.new()
    bmesh.ops.create_cube(bm, size=2.0)
    a = bm.verts.new((5, 5, 5))
    b = bm.verts.new((6, 5, 5))
    bm.edges.new((a, b))  # wire edge
    bm.verts.new((9, 9, 9))  # loose vert
    me = bpy.data.meshes.new("Loose")
    bm.to_mesh(me)
    bm.free()
    o = bpy.data.objects.new("Loose", me)
    bpy.context.collection.objects.link(o)
    c = btk.Diagnostics.find_problem_geometry(o, loose=True, select=False)
    check(
        "find_problem detects loose (1 wire edge + 1 vert)",
        c["loose"] == 2,
        f"loose={c['loose']}",
    )
    ...
    bpy.ops.mesh.primitive_cube_add()
    o = bpy.context.active_object
    c = btk.Diagnostics.find_problem_geometry(o, quads=True, select=True)
    check(
        "find_problem detects 6 quads on a cube", c["quads"] == 6, f"quads={c['quads']}"
    )
    ...
    bm.faces.new(vs)
    me = bpy.data.meshes.new("ZA")
    bm.to_mesh(me)
    bm.free()
    o = bpy.data.objects.new("ZA", me)
    bpy.context.collection.objects.link(o)
    c = btk.Diagnostics.find_problem_geometry(o, zero_area_faces=True, select=False)
    check(
        "find_problem detects 1 zero-area face",
        c["zero_area_faces"] == 1,
        f"za={c['zero_area_faces']}",
    )
    reset()
    bpy.ops.mesh.primitive_plane_add()
    o = bpy.context.active_object
    c = btk.Diagnostics.find_problem_geometry(o, zero_area_faces=True, select=False)
    check(
        "find_problem flat plane -> no zero-area face",
        c["zero_area_faces"] == 0,
        f"za={c['zero_area_faces']}",
    )
    ...
    bm = bmesh.new()
    bmesh.ops.create_cube(bm, size=2.0)
    a = bm.verts.new((3, 3, 3))
    b = bm.verts.new((3, 3, 3))
    bm.edges.new((a, b))  # coincident
    me = bpy.data.meshes.new("ZL")
    bm.to_mesh(me)
    bm.free()
    o = bpy.data.objects.new("ZL", me)
    bpy.context.collection.objects.link(o)
    c = btk.Diagnostics.find_problem_geometry(o, zero_length_edges=True, select=False)
    check(
        "find_problem detects 1 zero-length edge",
        c["zero_length_edges"] == 1,
        f"zl={c['zero_length_edges']}",
    )
    ...
    me = bpy.data.meshes.new("ZUV")
    bm.to_mesh(me)
    bm.free()
    o = bpy.data.objects.new("ZUV", me)
    bpy.context.collection.objects.link(o)
    c = btk.Diagnostics.find_problem_geometry(o, zero_uv_area=True, select=False)
    check(
        "find_problem detects 1 zero-UV-area face",
        c["zero_uv_area"] == 1,
        f"zuv={c['zero_uv_area']}",
    )
    ...
    bm.to_mesh(me)
    bm.free()
    o = bpy.data.objects.new("NoUV", me)
    bpy.context.collection.objects.link(o)
    c = btk.Diagnostics.find_problem_geometry(o, zero_uv_area=True, select=False)
    check(
        "find_problem zero-UV with no UV layer -> 0",
        c["zero_uv_area"] == 0,
        f"zuv={c['zero_uv_area']}",
    )
    ...
    me = bpy.data.meshes.new("OV")
    bm.to_mesh(me)
    bm.free()
    o = bpy.data.objects.new("OV", me)
    bpy.context.collection.objects.link(o)
    check(
        "get_overlapping_faces detects 1 coincident face",
        btk.get_overlapping_faces(o, select=True) == 1,
    )
    n2 = btk.get_overlapping_faces(o, delete=True)
    check(
        "get_overlapping_faces delete removes the dupe",
        n2 == 1 and len(o.data.polygons) == 1,
    ...
    check(
        "get_overlapping_duplicates finds 1 coincident cube (not the sphere)",
        len(dupes) == 1 and dupes[0] in (a, b),
        f"{[d.name for d in dupes]}",
    )
    reset()
    bpy.ops.mesh.primitive_cube_add(location=(0, 0, 0))
    bpy.ops.mesh.primitive_cube_add(location=(5, 0, 0))
    check(
        "get_overlapping_duplicates spaced cubes -> 0",
        btk.get_overlapping_duplicates() == [],
    )
    reset()
    bpy.ops.mesh.primitive_cube_add(location=(0, 0, 0))
    ...
    check(
        "get_overlapping_duplicates retain keeps the given, reports the twin",
    ...
    check(
        "get_similar_mesh matches the twin cube, not the sphere",
        c2 in res and sp not in res,
        f"{[o.name for o in res]}",
    )
    check(
        "get_similar_mesh selected the match only",
        c2.select_get() and not sp.select_get(),
    )
    check("get_similar_mesh excludes the reference by default", c1 not in res)
    check(
        "get_similar_mesh inc_orig includes the reference",
    ...
    b = bpy.context.active_object
    bpy.ops.mesh.primitive_cube_add(size=6, location=(12, 0, 0))
    big = bpy.context.active_object
    res = btk.get_similar_mesh([a], bounding_box=True, tolerance=0.01)
    check(
        "get_similar_mesh bbox matches same-size, excludes 3x",
        b in res and big not in res,
        f"{[o.name for o in res]}",
    )
    ...
    reset()
    bpy.ops.mesh.primitive_cube_add()
    one = bpy.context.active_object  # 1 shell
    ...
    check(
        "get_similar_mesh shell: 2-shell matches 2-shell, not 1-shell",
        tb in res and one not in res,
        f"{[o.name for o in res]}",
    )
    ...
    ob = bpy.data.meshes.new("OtherCube")
    _b = bmesh.new()
    bmesh.ops.create_cube(_b, size=2.0)
    _b.to_mesh(ob)
    _b.free()
    oc = bpy.data.objects.new("OtherCube", ob)
    other.collection.objects.link(oc)
    res = btk.get_similar_mesh([main_cube], face=True, select=True)
    check(
        "get_similar_mesh ignores other-scene objects (no crash, not matched)",
        oc not in res,
        f"{[o.name for o in res]}",
    )
    ...
    check(
        "separate_objects by_material -> 1 new part", len(new) == 1, f"new={len(new)}"
    )
    check(
        "separate_objects rename -> base + _part01",
    ...
    bm.to_mesh(me)
    bm.free()
    ob = bpy.data.objects.new("Two", me)
    bpy.context.collection.objects.link(ob)
    new = btk.separate_objects([ob], by_material=False)
    check(
        "separate_objects loose -> 1 new part from 2 shells",
        len(new) == 1,
        f"new={len(new)}",
    )
    ...
    check(
        "combine_objects plain -> single object",
        res is not None and res.name == "CombA",
        f"res={getattr(res, 'name', res)}",
    )
    check(
        "combine_objects plain -> 12 faces (6+6)",
        res is not None and faces(res) == 12,
        f"n={faces(res) if res else '?'}",
    )
    check(
        "combine_objects plain -> only one object left",
    ...
    reset()
    matX = bpy.data.materials.new("MatX")
    matY = bpy.data.materials.new("MatY")
    cubes = []
    ...
    for loc in ((0, 0, 0), (2, 0, 0), (100, 0, 0), (102, 0, 0)):
        ...
        cubes.append(c)
    res = btk.combine_objects(
        cubes, group_by_material=True, cluster_by_distance=True, threshold=10.0
    )
    check(
        "combine_objects cluster_by_distance -> 2 clusters",
        isinstance(res, list) and len(res) == 2,
        f"res={[o.name for o in res] if isinstance(res, list) else res}",
    )
    ...
    check(
        "combine_objects no-cluster -> 1 mesh for shared material",
    ...
    check(
        "loft -> a new mesh object",
        lofted is not None and lofted.type == "MESH",
        f"{getattr(lofted, 'type', lofted)}",
    )
    check(
        "loft -> (3 rings-1)*(4 pts-1) = 6 quads",
        lofted is not None and len(lofted.data.polygons) == 6,
    ...
    check(
        "loft section_spans=2 -> 12 quads",
        lofted is not None and len(lofted.data.polygons) == 12,
        f"faces={len(lofted.data.polygons) if lofted else '?'}",
    )
    ...
    check(
        "loft close=True -> 9 quads (periodic)",
        lofted is not None and len(lofted.data.polygons) == 9,
        f"faces={len(lofted.data.polygons) if lofted else '?'}",
    )
    ...
    bpy.ops.object.mode_set(mode="OBJECT")
    check(
        "detach_components separate -> 1 new object", len(new) == 1, f"new={len(new)}"
    )
    check(
        "detach_components separate -> new has 2 faces",
        new and faces(new[0]) == 2,
        f"n={faces(new[0]) if new else '?'}",
    )
    check(
        "detach_components separate -> original has 4 faces",
        faces(o) == 4,
        f"n={faces(o)}",
    )
    ...
    bpy.ops.object.mode_set(mode="OBJECT")
    check(
        "detach_components duplicate keeps original intact (6 faces)",
        faces(o) == 6,
        f"n={faces(o)}",
    )
    check(
        "detach_components duplicate -> new copy has 2 faces",
        new and faces(new[0]) == 2,
        f"n={faces(new[0]) if new else '?'}",
    )
    ...
    bpy.ops.object.mode_set(mode="OBJECT")
    check(
        "detach_components separate_each -> 2 objects", len(new) == 2, f"new={len(new)}"
    )
    check(
        "detach_components separate_each -> each has 1 face",
        all(faces(n) == 1 for n in new),
        f"faces={[faces(n) for n in new]}",
    )
    ...
    bpy.ops.object.mode_set(mode="OBJECT")
    check(
        "detach_components separate=False -> no new object",
        new == [] and len(bpy.data.objects) == before_n,
    ...
    # not the source cube's origin at world (0,0,0).
    from blendertk.xform_utils._xform_utils import XformUtils

    def origin_at_bbox_center(o):
        mn, mx = XformUtils.get_world_bbox(o)
        center = (mn + mx) / 2.0
        return (o.matrix_world.translation - center).length < 1e-4
    ...
    bpy.ops.object.mode_set(mode="OBJECT")
    check(
        "detach_components center_pivot -> each object centered separately",
        new and all(origin_at_bbox_center(n) for n in new),
        f"offsets={[round((XformUtils.get_world_bbox(n)[0] + XformUtils.get_world_bbox(n)[1]).length, 4) for n in new] if new else '?'}",
    )

    # center_pivot=False: origin left where the source had it (world origin), NOT recentered.
    # Single face so it's a real contrast: center_pivot=True would move the origin to (-1,0,0);
    # =False must leave it at (0,0,0).
    reset()
    bpy.ops.mesh.primitive_cube_add()
    o = bpy.context.active_object
    select_faces(o, [0])
    new = btk.detach_components(
        duplicate=False, separate=True, separate_each=False, center_pivot=False
    )
    bpy.ops.object.mode_set(mode="OBJECT")
    check(
        "detach_components center_pivot=False -> origin left at source (0,0,0)",
        new and new[0].matrix_world.translation.length < 1e-4,
        f"loc={tuple(round(v, 4) for v in new[0].matrix_world.translation) if new else '?'}",
    )

    # plain separate (separate_each=False, center_pivot default on): the single new object's
    # origin recenters on its OWN bbox. Use a single face so its bbox center is off world origin
    # (faces [0,1] are opposite -> symmetric extract centered at 0,0,0, which can't tell centered
    # from uncentered apart).
    reset()
    bpy.ops.mesh.primitive_cube_add()
    o = bpy.context.active_object
    select_faces(o, [0])
    ...
    bpy.ops.object.mode_set(mode="OBJECT")
    check(
        "detach_components separate center_pivot -> new object centered on its own bbox",
        new
        and origin_at_bbox_center(new[0])
        and new[0].matrix_world.translation.length > 1e-4,
        f"loc={tuple(round(v, 4) for v in new[0].matrix_world.translation) if new else '?'}",
    )
    ...
    o = bpy.context.active_object
    check(
        "detach_components in object mode -> [] (no crash)",
