...
try:
    ...
    check(
        "get reports the map",
        state is not None and os.path.basename(state["filepath"]) == "env_a.hdr",
        f"state={state}",
    )
    check("strength applied", abs(state["strength"] - 2.0) < 1e-6)
    check(
        "rotation roundtrips (degrees)",
        abs(state["rotation"] - 90.0) < 1e-4,
        f"rot={state['rotation']:.3f}",
    )
    check(
        "visible -> opaque background",
        state["visible"] and not bpy.context.scene.render.film_transparent,
    )
    ...
    check(
        "env -> background -> output linked",
        any(lk.from_node == env and lk.to_node == bg for lk in nt.links)
        and any(lk.from_node == bg and lk.to_node == out for lk in nt.links),
    )
    mapping = nt.nodes.get("btk_hdri_mapping")
    check(
        "mapping drives the env vector",
        any(lk.from_node == mapping and lk.to_node == env for lk in nt.links),
    )
    check(
        "mapping rotation in radians",
    ...
    check(
        "update reuses the node rig",
        len(nt.nodes) == node_count,
        f"{node_count} -> {len(nt.nodes)}",
    )
    state = btk.get_world_hdri()
    check("update swaps the map", os.path.basename(state["filepath"]) == "env_b.hdr")
    check(
        "invisible -> transparent film",
        not state["visible"] and bpy.context.scene.render.film_transparent,
    )
    ...
    state = btk.get_world_hdri()
    check(
        "levels-only update keeps the map",
        os.path.basename(state["filepath"]) == "env_b.hdr",
    )
    check(
        "levels-only update applies",
        abs(state["strength"] - 3.0) < 1e-6
        and abs(state["rotation"] - 45.0) < 1e-4
        and state["visible"],
    )
    ...
    check(
        "cleared nodes are gone",
        nt.nodes.get("btk_hdri_env") is None
        and nt.nodes.get("btk_hdri_mapping") is None,
    )
    check(
        "clear on an already-clear world returns False", btk.clear_world_hdri() is False
    )
    ...
    check(
        "ray visibility reports the applied state",
        rv is not None and rv["diffuse"] is False and rv["glossy"] is True,
        str(rv),
    )
    check(
        "get_world_ray_visibility round-trips",
    ...
    check(
        "partial ray-visibility update keeps glossy",
    ...
    bpy.context.scene.render.engine = (
        "CYCLES"  # world.cycles is a Cycles-addon namespace
    )
    applied = btk.set_world_importance_resolution(2048)
    w = bpy.context.scene.world
    check(
        "importance resolution switches to MANUAL and applies the map size",
        applied == 2048
        and w.cycles.sampling_method == "MANUAL"
        and w.cycles.sample_map_resolution == 2048,
        f"applied={applied}",
    )
    check(
        "get_world_importance_resolution round-trips in MANUAL",
        btk.get_world_importance_resolution() == 2048,
    )
    # 0/None restores AUTOMATIC sampling and reports None (Cycles sizes the map itself)
    check(
        "importance resolution 0 restores AUTOMATIC + reports None",
    ...
    check(
        "scale_light_energy reports every light's NEW power",
        scaled == {"KeyLight": 200.0, "RimLight": 50.0},
        f"{scaled}",
    )
    check(
        "relative brightnesses are preserved (4:1 before and after)",
        scaled["KeyLight"] / scaled["RimLight"] == 4.0,
    )
    check("1.0 is a no-op", btk.LightUtils.scale_light_energy(1.0) == scaled)
    ...
    check(
        "a shared light datablock scales once, not once per user",
        both == {"KeyLight": 400.0, "KeyLightCopy": 400.0},
        f"{both}",
    )
    check("an explicit subset leaves the others alone", rim.data.energy == 50.0)
    check(
        "no lights matched -> empty, no raise",
    ...
    check(
        "lights_from_records reports {record: object}",
        built == {"keyLight": "keyLight"},
        f"{built}",
    )
    ...
    check(
        "the lamp TOOK the empty's name (no .001 suffix)",
    ...
    check(
        "position comes from the placed empty",
    ...
    check(
        "area size = local extent x the empty's world scale",
    ...
    check(
        "radiance -> energy = radiance * pi * the lamp's REAL area",
    ...
    check(
        "a DISK's area is pi/4 of its bounding square",
    ...
    check(
        "an energy-only AREA record keeps its watts verbatim",
        abs(bpy.data.objects["plainArea"].data.energy - 42.0) < 1e-4,
        f"{bpy.data.objects['plainArea'].data.energy}",
    )

    check(
        "a record naming no object is skipped, not raised",
        btk.LightUtils.lights_from_records([{"name": "nope", "type": "POINT"}]) == {},
    )

    # A light the source authored but switched off arrives switched off, rather
    # than not arriving. A sender transferring a SCENE hands over authored
    # content, and an artist who hid a light to clear the viewport did not ask
    # for it to be deleted; a sender whose purpose is lighting something filters
    # before it sends, so this only ever fires on a transfer.
    for _name, _rec in (
        ("offLight", {"hidden": True}),
        ("onLight", {}),
    ):
        _e = bpy.data.objects.new(_name, None)
        bpy.context.scene.collection.objects.link(_e)
        _e.location = (3.0, -4.0, 5.0)
        btk.LightUtils.lights_from_records(
            [dict({"name": _name, "type": "POINT", "energy": 7.0}, **_rec)]
        )
    check(
        "a hidden record arrives as a hidden lamp, not as nothing",
        "offLight" in bpy.data.objects
        and bpy.data.objects["offLight"].type == "LIGHT"
        and bpy.data.objects["offLight"].hide_viewport
        and bpy.data.objects["offLight"].hide_render,
    )
    check(
        "a record with no hidden key arrives visible",
        not bpy.data.objects["onLight"].hide_viewport
        and not bpy.data.objects["onLight"].hide_render,
    )
    # A hidden object is NOT depsgraph-evaluated, so the hide flags are set only
    # after every transform is written -- a hidden lamp must still land exactly
    # where its empty stood, or the transferred scene is wrong the moment the
    # artist unhides it.
    bpy.context.view_layer.update()
    check(
        "a hidden lamp is still PLACED where its empty stood",
        all(
            abs(bpy.data.objects["offLight"].matrix_world.translation[i] - v) < 1e-5
            for i, v in enumerate((3.0, -4.0, 5.0))
        ),
        f"{tuple(bpy.data.objects['offLight'].matrix_world.translation)}",
    )
    ...
    bpy.context.view_layer.update()
    check(
        "a child of the light is re-parented onto it, not re-rooted",
        bpy.data.objects["lightChild"].parent is bpy.data.objects["hostLight"],
        f"{bpy.data.objects['lightChild'].parent}",
    )
    check(
        "the child keeps its world transform through the swap",
        all(
            abs(a - b) < 1e-5
            for a, b in zip(
    ...
    check(
        "world_emits: a lit background is a light source",
        btk.LightUtils.world_emits(w) is True,
    )

    bg.inputs["Strength"].default_value = 0.0
    check("world_emits: zero strength is not", btk.LightUtils.world_emits(w) is False)

    bg.inputs["Strength"].default_value = 1.0
    bg.inputs["Color"].default_value = (0.0, 0.0, 0.0, 1.0)
    check(
        "world_emits: a black background is not", btk.LightUtils.world_emits(w) is False
    )

    check(
        "world_emits: no world at all is not", btk.LightUtils.world_emits(None) is False
    )
    ...
    check(
        "world_emits: a linked background input is assumed lit",
        btk.LightUtils.world_emits(w) is True,
    )
    ...
    bg.inputs["Strength"].default_value = 0.0
    check(
        "world_emits: zero strength beats a linked colour (HDRI at 0 is dark)",
    ...
    w.use_nodes = False
    check(
        "world_emits: ignores the deprecated use_nodes flag (reads the tree)",
    ...
    check(
        "world_emits: defaults to the scene world", btk.LightUtils.world_emits() is True
    )
    bpy.context.scene.world = None
    check(
        "world_emits: a scene with no world is not lit by one",
    ...
    check(
        "set_world_environment creates a world when the scene has none",
        world is not None and world.node_tree is not None,
        f"{world}",
    )
    bg = next((n for n in world.node_tree.nodes if n.type == "BACKGROUND"), None)
    check(
        "flat ambient wires a Background node with the requested colour/strength",
        bg is not None
        and all(
            abs(a - b) < 1e-5
            for a, b in zip(bg.inputs["Color"].default_value[:3], (0.1, 0.2, 0.3))
        )
        and abs(bg.inputs["Strength"].default_value - 2.0) < 1e-5,
        f"{tuple(bg.inputs['Color'].default_value) if bg else None}",
    )
    check(
        "the Background reaches the World Output", bool(bg.outputs["Background"].links)
    )
    check("it reports what it applied", "flat ambient" in desc and "2.0" in desc, desc)
    check(
        "a flat-ambient world is a light source",
    ...
    check(
        "a second call updates in place (one Background node)",
        len(bgs) == 1,
