"""blendertk target_weld (btk.TargetWeld.activate) headless test.
...
try:
    ...
    # ---- surface: class-only (btk.TargetWeld.activate; no flat btk.target_weld) ------------
    check(
        "btk.TargetWeld.activate resolves (class-only)",
        callable(getattr(btk.TargetWeld, "activate", None))
        and not hasattr(btk, "target_weld"),
    )

    # ---- pure math: project_points ---------------------------------------------------------
    identity = np.eye(4)
    xy, depth = tw.TargetWeld.project_points(identity, [(0.0, 0.0, 0.5)], 200, 100)
    check(
        "project_points maps NDC origin to region center",
        np.allclose(xy[0], (100.0, 50.0)),
        f"xy={xy[0]}",
    )
    behind = np.eye(4)
    behind[3, :] = (0, 0, 0, -1.0)  # forces w < 0
    xy_b, depth_b = tw.TargetWeld.project_points(behind, [(0.0, 0.0, 0.0)], 200, 100)
    check(
        "project_points flags behind-camera as NaN/inf",
    ...
    check(
        "pick: nearest within radius wins",
        tw.TargetWeld.pick_screen_point((49, 50), pts, dep, radius=14) == 1,
    )
    check(
        "pick: nothing within radius -> None",
        tw.TargetWeld.pick_screen_point((150, 150), pts, dep, radius=14) is None,
    )
    dep_front = np.array([0.5, 0.9, 0.1])  # idx 2 nearer the viewer
    check(
        "pick: front-most wins over screen-nearest",
        tw.TargetWeld.pick_screen_point((49, 50), pts, dep_front, radius=14) == 2,
    )
    check(
        "pick: exclude removes the armed source",
        tw.TargetWeld.pick_screen_point((49, 50), pts, dep, radius=14, exclude=1) == 2,
    )
    check(
        "pick: empty input -> None",
        tw.TargetWeld.pick_screen_point((0, 0), np.empty((0, 2)), np.empty(0)) is None,
    )

    # ---- pure math: weld_position / dash_segments ------------------------------------------
    check(
        "weld_position at target",
        tw.TargetWeld.weld_position((0, 0, 0), (2, 4, 6)) == (2.0, 4.0, 6.0),
    )
    check(
        "weld_position at center",
        tw.TargetWeld.weld_position((0, 0, 0), (2, 4, 6), merge_to_center=True)
        == (1.0, 2.0, 3.0),
    )
    dashes = tw.TargetWeld.dash_segments((0, 0), (100, 0), dash=6, gap=4)
    check(
        "dash_segments returns paired endpoints",
        len(dashes) >= 2 and len(dashes) % 2 == 0,
        f"n={len(dashes)}",
    )
    check(
        "dash_segments spans the full run",
        dashes[0] == (0.0, 0.0) and abs(dashes[-1][0] - 100.0) <= 6.0,
        f"end={dashes[-1]}",
    )
    check(
        "dash_segments zero-length -> empty",
        tw.TargetWeld.dash_segments((5, 5), (5, 5)) == [],
    )
    check(
        "prompt text reflects mode",
        "merge at target"
        in tw.TargetWeld._prompt_text(
    ...
    tw.TargetWeld.weld_pair(bm, v_src, v_tgt, merge_to_center=False)
    check(
        "edge-connected weld removes one vert",
        len(bm.verts) == 7,
        f"verts={len(bm.verts)}",
    )
    check(
        "welded vert sits at the target",
    ...
    tw.TargetWeld.weld_pair(bm, v_src, v_tgt, merge_to_center=True)
    check("center weld removes one vert", len(bm.verts) == 7)
    check(
        "center weld sits at the midpoint",
    ...
    def ev(etype, value="PRESS", alt=False):
        return types.SimpleNamespace(
            type=etype, value=value, alt=alt, mouse_region_x=0, mouse_region_y=0
        )

    op = op_stub()
    check(
        "modal: Alt+LMB (tumble) passes through",
        tw.TargetWeld._op_modal(op, bpy.context, ev("LEFTMOUSE", alt=True))
        == {"PASS_THROUGH"},
    )
    check(
        "modal: Alt+RMB (dolly) passes through instead of exiting",
        tw.TargetWeld._op_modal(op, bpy.context, ev("RIGHTMOUSE", alt=True))
        == {"PASS_THROUGH"},
    )
    check(
        "modal: MMB orbit passes through",
        tw.TargetWeld._op_modal(op, bpy.context, ev("MIDDLEMOUSE")) == {"PASS_THROUGH"},
    )
    check(
        "modal: wheel zoom passes through",
        tw.TargetWeld._op_modal(op, bpy.context, ev("WHEELUPMOUSE"))
        == {"PASS_THROUGH"},
    )
    ...
    check(
        "modal: Alt-nav abandons the in-flight drag (no ghost rubber band)",
        tw.TargetWeld._op_modal(op, bpy.context, ev("LEFT_ALT", alt=True))
        == {"PASS_THROUGH"}
        and op._source is None
        and op._hover is None,
    )
    op._source, op._hover = dict(hit), dict(hit)
    check(
        "modal: MMB nav drops the stale hover but keeps the armed source",
        tw.TargetWeld._op_modal(op, bpy.context, ev("MIDDLEMOUSE")) == {"PASS_THROUGH"}
        and op._source is not None
        and op._hover is None,
    )
    op._source = None

    check(
        "modal: plain Esc still exits (CANCELLED with no welds)",
        tw.TargetWeld._op_modal(op, bpy.context, ev("ESC")) == {"CANCELLED"},
    )

    # ---- operator registration (lazy + idempotent) -----------------------------------------
    tw.TargetWeld._ensure_operator()
    check("operator registers", hasattr(bpy.types, "BTK_OT_target_weld"))
    tw.TargetWeld._ensure_operator()  # second call must be a no-op
