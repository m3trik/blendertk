...
try:
    ...
    # ---- surface: engines are class-only (not flat on btk), matching mayatk
    check(
        "duplicate engines are class-only (not flat on btk)",
        callable(btk.DuplicateLinear.duplicate_linear)
        and callable(btk.DuplicateRadial.duplicate_radial)
        and callable(btk.DuplicateGrid.duplicate_grid)
        and not any(
            hasattr(btk, n)
            for n in ("duplicate_linear", "duplicate_radial", "duplicate_grid")
        ),
    )
    ...
    reset()
    o = cube_at()
    out = btk.DuplicateLinear.duplicate_linear(
        o, 4, translate=(8, 0, 0), calculation_mode="linear"
    )
    copies = out[o]
    ...
    check(
        "linear positions ramp to the full offset",
        all(abs(a - b) < 1e-4 for a, b in zip(xs, expect)),
    ...
    o = cube_at()
    out = btk.DuplicateLinear.duplicate_linear(
        o, 2, translate=(4, 0, 0), calculation_mode="linear", instance=False
    )
    ...
    o = cube_at(x=2.0)
    out = btk.DuplicateLinear.duplicate_linear(
        o, 1, rotate=(0, 0, 180), pivot="world", calculation_mode="linear"
    )
    c = out[o][0]
    t = c.matrix_world.translation
    check(
        "linear 180-deg world orbit lands at -x",
        abs(t.x + 2.0) < 1e-4 and abs(t.y) < 1e-4,
        f"({t.x:.2f},{t.y:.2f})",
    )
    ...
    o = cube_at()
    out = btk.DuplicateLinear.duplicate_linear(
        o, 2, scale=(2, 2, 2), calculation_mode="linear"
    )
    s_last = out[o][-1].matrix_world.to_scale()
    check(
        "linear scale ramps to the full factor",
        abs(s_last.x - 2.0) < 1e-3,
        f"{s_last.x:.3f}",
    )
    ...
    reset()
    o = cube_at(x=3.0)
    out = btk.DuplicateRadial.duplicate_radial(
        o, 4, rotate_axis="z", pivot="world", keep_original=True
    )
    copies = out[o.name]
    ...
    angles = sorted(
        (
            math.degrees(
                math.atan2(c.matrix_world.translation.y, c.matrix_world.translation.x)
            )
            + 360
        )
        % 360
    ...
    check(
        "radial full revolution spaces 90 deg",
        all(abs(a - b) < 1e-3 for a, b in zip(angles, expect)),
    ...
    name = o.name
    out = btk.DuplicateRadial.duplicate_radial(
        o, 3, end_angle=180, rotate_axis="z", pivot="world"
    )
    check(
        "radial drops the original",
        name not in bpy.data.objects or bpy.data.objects[name].type == "EMPTY",
    )
    empties = [e for e in bpy.data.objects if e.type == "EMPTY"]
    check(
        "radial groups copies under an Empty",
        len(empties) == 1 and all(c.parent is empties[0] for c in out[name]),
    ...
    check(
        "radial arc keeps endpoints (0/90/180)",
    ...
    name = o.name
    out = btk.DuplicateRadial.duplicate_radial(
        o, 4, rotate_axis="z", pivot="world", combine=True
    )
    res = out[name]
    check(
        "radial combine yields one mesh of 4 cubes",
    ...
    out = btk.DuplicateGrid.duplicate_grid(o, dimensions=(2, 2, 1), spacing=1.0)
    copies = out[o]
    check("grid 2x2x1 makes 3 copies", len(copies) == 3, f"n={len(copies)}")
    locs = sorted(
        (round(c.matrix_world.translation.x, 3), round(c.matrix_world.translation.y, 3))
        for c in copies
    )
    check(
        "grid steps = bbox+spacing",
        locs == [(0.0, 3.0), (3.0, 0.0), (3.0, 3.0)],
        f"{locs}",
    )
    check(
        "grid copies grouped under an Empty",
    ...
    # ---- grid per-axis spacing tuple (sx, sy, sz) sets the gap independently
    reset()
    o = cube_at(size=2.0)  # bbox 2 units
    out = btk.DuplicateGrid.duplicate_grid(
        o, dimensions=(2, 2, 1), spacing=(1.0, 3.0, 0.0)
    )
    locs = sorted(
        (round(c.matrix_world.translation.x, 3), round(c.matrix_world.translation.y, 3))
        for c in out[o]
    )
    # X step = bbox(2) + 1 = 3 ; Y step = bbox(2) + 3 = 5
    check(
        "grid per-axis spacing",
        locs == [(0.0, 5.0), (3.0, 0.0), (3.0, 5.0)],
        f"{locs}",
    )
    check(
        "grid scalar spacing == uniform tuple",
        btk.DuplicateGrid._normalize_spacing(2) == (2.0, 2.0, 2.0)
        and btk.DuplicateGrid._normalize_spacing((1, 2, 3)) == (1.0, 2.0, 3.0),
    )

    # ---- grid negative dimension lays out the other way
    reset()
    o = cube_at(size=2.0)
    out = btk.DuplicateGrid.duplicate_grid(o, dimensions=(-2, 1, 1), spacing=0.0)
    ...
    reset()
    o = cube_at(size=2.0)
    out = btk.DuplicateGrid.duplicate_grid(o, dimensions=(3, 1, 1), mode="combine")
    res = out[o]
    check(
        "grid combine joins 2 copies into one mesh",
    ...
    o = cube_at()
    try:
        btk.DuplicateGrid.duplicate_grid(o, dimensions=(100, 100, 100))
