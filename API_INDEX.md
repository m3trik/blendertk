...
- `set_visibility_keys(objects, visible=True, frame=None, when='current', offset=0)`
- `add_intermediate_keys(objects, step=1.0, time_range=None, ignore_visibility=False)`
...
- `invert_keys(objects, mode='time', value_pivot=0.0)`
- `snap_keys(objects, selected_only=False, time_range=None)`
...
- `bake_keys(objects=None, frame_range=None, step=1, only_selected=False, visual_keying=True, clear_constraints=False, clear_parents=False, bake_types=None)`
- `bake_blend_shapes(objects=None, frame_range=None, step=1)`
- `get_animation_info(objects=None, by_time=False)`
...
- `scale_keys(objects, factor, pivot=None)`
- `class ScaleKeys`
...
- `class CurtainSlots(ptk.LoggingMixin)`
  - methods: header_init, cmb000_init, perform_operation, b001, b002
...
- `duplicate_linear(objects, num_copies, translate=(0, 0, 0), rotate=(0, 0, 0), scale=(1, 1, 1), weight_bias=0.5, weight_curve=4.0, pivot='object', calculation_mode='weighted', instance=True)`
...
- `duplicate_radial(objects, num_copies, start_angle=0.0, end_angle=360.0, weight_bias=0.5, weight_curve=0.5, rotate_axis='y', offset=(0, 0, 0), translate=(0, 0, 0), rotate=(0, 0, 0), scale=(1, 1, 1), pivot='object', keep_original=False, instance=False, combine=False)`
...
- `class MacroManager`
  - methods: set_macros, call_with_input, set_macro, remove_macros
...
- `class NamingSlots(Naming, ptk.LoggingMixin)`
  - methods: valid_suffixes, header_init, txt000_init, txt000, txt001_init, txt001, tb000_init, tb000, tb001_init, tb001, tb002_init, tb002, tb003_init, tb003
...
- `set_world_hdri(filepath=None, strength=1.0, rotation=0.0, visible=True)`
- `get_world_hdri()`
- `set_world_ray_visibility(diffuse=None, glossy=None)`
- `get_world_ray_visibility()`
- `class LightUtils`

### `light_utils/hdr_manager.py` — HDR Manager tool panel — Switchboard slot wiring for the co-located ``hdr_manager.ui``.
- `class HdrManagerSlots(ptk.LoggingMixin)`
  - methods: header_init, cmb000_init, txt000, b001, cmb000, spn_intensity, spn_exposure, slider000, chk000, chk_diffuse, chk_glossy, reveal_selected, open_folder
...
- `resolve_missing_textures(search_dir, recursive=True, stem=False, texture=False, fuzzy=False)`
- `normalize_texture_paths(mode='relative', project_dir=None)`
...
- `class GameShaderSlots(ptk.LoggingMixin)`
  - methods: header_init, cmb001_init, b000, b001
...
### `mat_utils/image_to_plane/image_to_plane_slots.py` — Switchboard slots for the Image to Plane panel — port of mayatk's ``ImageToPlaneSlots``.
...
- `class MatUpdaterSlots(MatUpdater)`
  - methods: header_init, cmb001_init, b001
...
- `class ShaderTemplatesSlots(ptk.LoggingMixin)`
  - methods: header_init, cmb002_init, b000, b001, save_template, load_textures, rename_template, delete_template, open_templates_folder
...
- `class TexturePathEditorSlots(ptk.LoggingMixin)`
  - methods: header_init, tb_set_texture_directory_init, tb_find_and_copy_textures_init, tb_normalize_paths_init, tb_resolve_missing_textures_init, tbl000_init, open_textures_folder, reload_scene_textures, tb_set_texture_directory, tb_find_and_copy_textures, tb_normalize_paths, tb_resolve_missing_textures, select_textures_for_objects, select_broken_paths, select_absolute_paths, row_browse_for_file, select_material, row_show_in_shader_editor, delete_image, handle_cell_edit, refresh_texture_table
...
- `class ChannelsSlots`
  - methods: cmb000_init, cmb000, header_init, show_create_menu, tbl000_init

### `node_utils/data_nodes.py` — Scene-wide export-metadata carrier — mirror of mayatk's ``node_utils.data_nodes``.
- `class DataNodes`
  - methods: get_export_node, set_export_string, get_export_string
...
- `class NurbsUtils(ptk.LoggingMixin)`
  - methods: add_spline, create_curve, curve_to_mesh
...
- `class TubePath`
  - methods: get_centerline, get_centerline_using_edges

### `rig_utils/tube_rig.py` — Tube Rig — Blender port of mayatk's ``rig_utils.tube_rig`` (the engine + strategies).
...
- `class TubeRig(ptk.LoggingMixin)`
  - methods: collection, resolve_centerline, create_root, create_armature, build_curve, make_control, hook_curve_controls, build
...
- `class WheelRig(ptk.LoggingMixin)`
  - methods: rig_name, rig_rotation
- `class WheelRigSlots(ptk.LoggingMixin)`
  - methods: cmb000_init, s000_init, header_init, wheel_rig
...
- `transform_uvs(objects, flip_u=False, flip_v=False, angle=0.0)`
