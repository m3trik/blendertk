...
analogous CLI; its automation surface is ``--mesh`` + the ``--enable-remote-scripting``
JSON-RPC port. So Substance templates are *descriptive* (metadata constants parsed via
...
from blendertk.mat_utils.substance_bridge.connection import (
    default_log_path,
    find_painter_exe,
    SubstanceConnection,
)
