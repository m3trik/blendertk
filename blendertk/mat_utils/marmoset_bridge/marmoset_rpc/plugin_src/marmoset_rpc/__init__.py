...
Toolbag (e.g. tentacle/mayatk/blendertk slot discovery, which imports
every ``*.py`` under the package to introspect classes) is inert and
never binds a port. ``MARMOSET_RPC_AUTOSTART=0`` forces it off even
inside Toolbag, which unit tests rely on. See :func:`.server.autostart`.
"""
