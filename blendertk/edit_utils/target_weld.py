...
- The operator class is built lazily inside ``_ensure_operator()`` (the ``macros.py`` pattern)
  ...
  surface (``btk.TargetWeld.activate``) — never needs a running Blender. The pure geometry helpers
...
class _TargetWeldInternal(object):
    """Internal helpers for TargetWeld."""
    ...
    def _pick(op, context, mouse, exclude=None):
        ...
        best = None
        for obj in _TargetWeldInternal._edit_meshes(context):
            cache = _TargetWeldInternal._cache_for(op, obj)
            if len(cache["coords"]) == 0:
                continue
            mvp = persp @ np.array(obj.matrix_world, dtype=np.float64)
            xy, depth = TargetWeld.project_points(
                mvp, cache["coords"], region.width, region.height
            )
            excl = None
            ...
            row = TargetWeld.pick_screen_point(
                mouse,
                xy,
                depth,
                PICK_RADIUS * _TargetWeldInternal._ui_scale(),
                exclude=excl,
            )
            if row is None:
                continue
            hit = {
                ...
                "co": tuple(
                    cache["coords"][row]
                ),  # local-space — survives nav mid-drag
    ...
    def _do_weld(op, context, source, target) -> bool:
        ...
        if (v_src.co - Vector(source["co"])).length > 1e-6:
            op._caches.pop(obj.name, None)
            return False
        TargetWeld.weld_pair(bm, v_src, v_tgt, op.merge_to_center)
    ...
    def _drop_transients(op, context, drop_source: bool) -> None:
        ...
        if drop_source:
            op._source = None
        _TargetWeldInternal._tag_redraw(context)
    ...
    def _op_invoke(op, context, event):
        ...
        if (
            ...
            return {"CANCELLED"}
        if not _TargetWeldInternal._edit_meshes(context):
        ...
        op._handle = bpy.types.SpaceView3D.draw_handler_add(
            _TargetWeldInternal._draw, (op,), "WINDOW", "POST_PIXEL"
        )
    ...
    def _op_modal(op, context, event):
        ...
        if not _TargetWeldInternal._edit_meshes(context):
            return _TargetWeldInternal._finish(op, context)
        ...
        if event.alt:
            _TargetWeldInternal._drop_transients(op, context, drop_source=True)
            return {"PASS_THROUGH"}

        etype = event.type
        if etype in _NAV_EVENTS or (
            etype.startswith("NUMPAD")
            and etype != "NUMPAD_ENTER"  # numpad = view nav keys
        ):
            ...
            _TargetWeldInternal._drop_transients(op, context, drop_source=False)
            return {"PASS_THROUGH"}

        if etype in {"MOUSEMOVE", "INBETWEEN_MOUSEMOVE"}:
            op._mouse = (event.mouse_region_x, event.mouse_region_y)
            exclude = (
                (op._source["obj"], op._source["index"])
                if op._source is not None
                else None
            )
            op._hover = _TargetWeldInternal._pick(
                op, context, op._mouse, exclude=exclude
            )
            _TargetWeldInternal._tag_redraw(context)
            return {"PASS_THROUGH"}

        if etype == "LEFTMOUSE":
            if not _TargetWeldInternal._in_region(
                context, event
            ):  # clicks outside the viewport aren't ours
                return {"PASS_THROUGH"}
            op._notice = ""
            op._mouse = (event.mouse_region_x, event.mouse_region_y)
            if event.value == "PRESS":
                op._caches.clear()  # pass-through edits may have moved verts — repick fresh
                op._source = _TargetWeldInternal._pick(op, context, op._mouse)
                op._hover = None
            elif event.value == "RELEASE" and op._source is not None:
                target = _TargetWeldInternal._pick(
                    op,
                    context,
                    op._mouse,
                    exclude=(op._source["obj"], op._source["index"]),
                )
                if target is not None:
                    _TargetWeldInternal._do_weld(op, context, op._source, target)
                op._source = None
                op._hover = _TargetWeldInternal._pick(op, context, op._mouse)
            _TargetWeldInternal._tag_redraw(context)
            return {"RUNNING_MODAL"}
    ...
    def _draw(op):
        ...
        context = bpy.context
        if (
            context.region != op._region
        ):  # the handler fires for every 3D view — ours only
            return

        shader = gpu.shader.from_builtin("UNIFORM_COLOR")
        gpu.state.blend_set("ALPHA")
        scale = _TargetWeldInternal._ui_scale()
        ...
        def marker(xy, color):
            tris(
                _TargetWeldInternal._square(xy[0], xy[1], HALO_SIZE * scale), COLOR_HALO
            )
            tris(_TargetWeldInternal._square(xy[0], xy[1], MARKER_SIZE * scale), color)
        ...
        if op._source is not None:
            src_xy = _TargetWeldInternal._source_xy(op, context)
            if src_xy is not None:
                end = op._hover["xy"] if op._hover is not None else op._mouse
                dashes = TargetWeld.dash_segments(
                    src_xy, end, dash=DASH_LEN * scale, gap=GAP_LEN * scale
                )
                if dashes:
        ...
        text = _TargetWeldInternal._prompt_text(op)
        font_id = 0
    ...
    def _ensure_operator():
        ...
        class BTK_OT_target_weld(bpy.types.Operator):
            ...
            def invoke(self, context, event):
                return _TargetWeldInternal._op_invoke(self, context, event)

            def modal(self, context, event):
                return _TargetWeldInternal._op_modal(self, context, event)

            def cancel(self, context):
                _TargetWeldInternal._finish(self, context)
...
class TargetWeld(_TargetWeldInternal):
    ...
    @staticmethod
    def activate(merge_to_center: bool = False) -> bool:
        """Public entry point — start the interactive Target Weld tool (see :meth:`target_weld`)."""
        return TargetWeld.target_weld(merge_to_center)
    ...
    def weld_pair(bm, v_src, v_tgt, merge_to_center: bool = False) -> None:
        ...
        co = Vector(TargetWeld.weld_position(v_src.co, v_tgt.co, merge_to_center))
    ...
    def target_weld(merge_to_center: bool = False) -> bool:
        ...
        _TargetWeldInternal._ensure_operator()
        ...
        ctx = CoreUtils.get_view3d_context()
        if not ctx or not ctx.get("region"):
            raise RuntimeError(
                "No 3D viewport available — open a 3D view to use Target Weld."
            )
        with bpy.context.temp_override(**ctx):
            ...
            if active is None or active.type != "MESH":
                ...
                if not meshes:
                    raise RuntimeError(
                        "Target Weld requires an active or selected mesh object."
                    )
                active = meshes[0]
