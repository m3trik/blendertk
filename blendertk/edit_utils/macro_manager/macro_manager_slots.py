...
``edit_utils.macro_manager`` (mirror at the name + behavior level).

``MacroManagerSlots`` is a single-table Switchboard interface for assigning a
hotkey and category to any ``blendertk`` macro (``blendertk.edit_utils.macros.Macros``),
filtering the list, and saving / loading named binding sets as presets. All
non-UI logic is delegated to :class:`blendertk.edit_utils.macros.MacroManager`,
the single source of truth, which is fully usable without this panel (see its
own module docstring).

Layout (identical to mayatk's panel — same objectNames, same table columns):
...
Divergence from mayatk (by design): Maya's hotkey registry
(``cmds.runTimeCommand`` / ``cmds.hotkey``) is a DCC-side store Maya itself
persists across sessions, so the panel re-queries it live on every show.
Blender's addon keyconfig only lives for the current process (see
``MacroManager``'s class docstring); "live" here means "what THIS session's
``Macros._KEYMAPS`` bookkeeping list currently holds", refreshed the same way
(:meth:`_reload_bindings`, wired to ``on_show``) so editing bindings elsewhere
in the same session (or a fresh ``apply_saved_macros()`` at startup) is always
reflected. Discovered by ``BlenderUiHandler`` (``marking_menu.show("macro_manager")``).
"""
import os
...
_CONFLICT_COLOR = "#c86464"  # desaturated red — matches mayatk's channels panel


class MacroManagerSlots(ptk.LoggingMixin):
    """Switchboard slots for the Macro Manager UI.

    Self-contained (``ptk.LoggingMixin`` only) so blendertk carries no
    back-dependency on tentacle. All state lives on ``self.controller``
    (:class:`blendertk.edit_utils.macros.Macros`) — this class only renders it.
    """
    ...
    def __init__(self, switchboard, log_level="WARNING"):
        ...
        self.logger.set_log_prefix("[macro_manager] ")
        ...
        # Re-query the live bindings every time the panel is shown so the table
        # reflects anything changed elsewhere in this session (see module docstring).
        self.ui.on_show.connect(self._reload_bindings)
    ...
    def header_init(self, widget):
        ...
        widget.set_help_text(
            fmt(
                title="Macro Manager",
                body="Assign a hotkey and category to any blendertk macro, filter "
    ...
    def _init_presets(self, header_widget):
        ...
        try:
            ...
            self.presets = PresetManager(
                parent=self.ui,
                preset_dir="blendertk/macro_manager",
            ...
            self.presets = None
            self.logger.warning(f"preset combo unavailable: {error}")
    ...
    def _import_bindings(self, data):
        """value_applier — apply a loaded binding set to Blender + memory."""
        data = data or {}
    ...
    def _load_bindings(self):
        """Seed the available macros + live bindings queried from the controller."""
    ...
    def _reload_bindings(self):
        """Re-query the live bindings and rebuild the table (on panel show)."""
        self._load_bindings()
        self.cmb000_init(self.ui.cmb000)
        self._refresh_table()
        self._mark_modified()
    ...
    def _on_hotkey_captured(self, row, _col, sequence):
        ...
        convert it to the engine's Maya-style hotkey notation before persisting.
        """
    ...
    def _on_category_captured(self, row, _col, new_cat):
        ...
        closed: rebuilding the table here is safe (unlike a persistent combo)
        and keeps the category filter + filtered view in sync.
        """
        name = self._row_macro(row)
        if not name:
            return
