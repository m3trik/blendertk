...
# problem-detection lives in ``Diagnostics`` not ``EditUtils``. Reached as
# ``btk.Diagnostics.find_problem_geometry`` (class-only — diagnostics is a submodule, not a flat
# ``_*_utils`` root); ``mesh_diag`` imports ``_bmesh_each`` / ``_object_mode`` here.
...
class _EditUtilsInternal(object):
    ...
    def _crease_layer(bm):
        ...
        return bm.edges.layers.float.get("crease_edge") or bm.edges.layers.float.new(
            "crease_edge"
        )
    ...
    def _copy_object(src, instance=True):
        ...
        _EditUtilsInternal._link_like(new, src)
        return new
    ...
    def _group_under_empty(copies, name, center=False):
        ...
        _EditUtilsInternal._link_like(empty, copies[0])
        if center:
            import mathutils
    ...
    def _mesh_metrics(obj, flags):
        ...
        if "shell" in flags:
            bm = bmesh.new()
            bm.from_mesh(me)
            m["shell"] = _EditUtilsInternal._count_shells(bm)
            bm.free()
    ...
    def _material_key(obj):
        ...
        mats = _EditUtilsInternal._materials_of(obj)
        if not mats:
            return "None"
        if len(mats) == 1:
            return mats[0]
...
class EditUtils(_EditUtilsInternal):
    ...
    def hook_curve_point(curve, point_index, target, name=None, falloff_type="NONE"):
        ...
        mod.matrix_inverse = EditUtils.hook_bind_inverse(target, curve)
        return mod
    ...
    def triangulate(objects):
        ...
        _EditUtilsInternal._bmesh_each(
            objects, lambda bm: bmesh.ops.triangulate(bm, faces=bm.faces[:])
        )
    ...
    def tris_to_quads(objects, angle=40.0):
        ...
        def _join(bm):
            bmesh.ops.join_triangles(
                bm,
                faces=bm.faces[:],
                angle_face_threshold=limit,
                angle_shape_threshold=limit,
                cmp_seam=False,
                cmp_sharp=False,
                cmp_uvs=False,
                cmp_materials=False,
        ...
        _EditUtilsInternal._bmesh_each(objects, _join)
    ...
    def subdivide_mesh(objects, cuts=1):
        ...
        def _sub(bm):
            bmesh.ops.subdivide_edges(
                bm, edges=bm.edges[:], cuts=cuts, use_grid_fill=True
            )

        _EditUtilsInternal._bmesh_each(objects, _sub)
    ...
    def boolean_op(objects, operation="DIFFERENCE", apply=True):
        ...
        objs = _EditUtilsInternal._meshes(objects)
        if len(objs) < 2:
            return None
        ...
        for operand in operands:
            ...
            mod.object = operand
            if apply:
                _EditUtilsInternal._apply_modifier(base, mod.name)
        return base
    ...
    def apply_subdivision(objects):
        ...
        for o in _EditUtilsInternal._meshes(objects):
            ...
            mod = next(
                (m for m in o.modifiers if m.type == "SUBSURF" and m.show_viewport),
                None,
            )
            if mod is not None:
                _EditUtilsInternal._apply_modifier(o, mod.name)
    ...
    @staticmethod
    @CoreUtils._object_mode  # bmesh on obj.data reads/writes STALE data while the object is in Edit Mode
    def average_normals(objects, by_uv_shell=False):
        ...
        for o in _EditUtilsInternal._meshes(objects):
            ...
            if by_uv_shell and uv is not None:
                for edge in bm.edges:
                    if _EditUtilsInternal._edge_is_uv_seam(edge, uv):
    ...
    def set_edge_hardness(objects, angle=30.0, upper_hardness=0, lower_hardness=180):
        ...
        def _smooth_for(
            hardness,
        ):  # -> edge.smooth value, or None to leave the edge unchanged
            if hardness is None:
                return None
            return (
                hardness >= 90.0
            )  # smooth (soft) when leaning soft, sharp (hard) when leaning hard
    ...
    def clear_custom_split_normals(objects):
        ...
        """
        return _EditUtilsInternal._set_custom_split_normals(objects, add=False)
    ...
    def add_custom_split_normals(objects):
        ...
        """
        return _EditUtilsInternal._set_custom_split_normals(objects, add=True)
    ...
    def clean_geometry(
        ...
        *,
        ...
        processed = _EditUtilsInternal._meshes(objects)
        _EditUtilsInternal._bmesh_each(processed, _clean)
        return processed
    ...
    def crease_edges(objects, amount=10.0, angle=None):
        ...
        **Mode-aware** (so it is NOT ``@_object_mode``-guarded): in EDIT mode it acts on the *selected*
        ...
        def _apply(bm, selected_only):
            ...
            cl = _EditUtilsInternal._crease_layer(bm)
            for e in bm.edges:
        ...
        for o in _EditUtilsInternal._meshes(objects):
            if o.mode == "EDIT":
                ...
                _EditUtilsInternal._bmesh_edit(
                    o, lambda bm: _apply(bm, selected_only=False)
                )
        return value
    ...
    def mirror(
        ...
        for obj in _EditUtilsInternal._meshes(objects):
            ...
            if merge_mode < 0:
                ...
                _EditUtilsInternal._bmesh_edit(new, _reflect_all)
                out.append(new)
                # Separate mode doesn't combine — center only the fresh half so its
                # origin sits on itself, not floating over the (untouched) source.
                to_center.append(new)
                if delete_original:
                    data = obj.data
                ...
                weld = (
                    _EditUtilsInternal._local_bisect_plane(obj, point, normal)
                    if merge_mode >= 1
                    else None
                )
                _EditUtilsInternal._bmesh_edit(
                    obj,
                    lambda bm, r=refl, w=weld: _EditUtilsInternal._duplicate_reflect(
                        bm, r, weld=w, threshold=merge_threshold
                    ),
                )
                out.append(obj)
                # Combined into this same object — its old origin is now off to one
                # side of the doubled result, so center it (allowed: it IS the result).
                to_center.append(obj)
        if center_pivot and to_center:
            # Re-center on the bounding box (BOUNDS = Maya's centerPivots); the shared
            # helper also drops any stale freeze-location bake the moved origin invalidates.
            from blendertk.xform_utils._xform_utils import XformUtils

            XformUtils.center_pivot(to_center, mode="object")
        return out
    ...
    def cut_along_axis(
        ...
        for obj in _EditUtilsInternal._meshes(objects):
            ...
            def _cut(bm, obj=obj, rel=rel, point=point, normal=normal):
                for d in rel:
                    p_l, n_l = _EditUtilsInternal._local_bisect_plane(
                        obj, point + d * normal, normal
                    )
                ...
                if delete:
                    ...
                    if mirror:
                        refl = _EditUtilsInternal._local_reflection(obj, p_deep, normal)
                        _EditUtilsInternal._duplicate_reflect(
                            bm,
                            refl,
                            weld=_EditUtilsInternal._local_bisect_plane(
                                obj, p_deep, normal
                            ),
                            threshold=max(merge_threshold, 1e-5),
                        )

            _EditUtilsInternal._bmesh_edit(obj, _cut)
            if delete and mirror:
                symmetrized.append(obj)
        # Symmetrize combines the reflected half back into the object; its old origin is now
        # off the center of the doubled result, so re-center it on the bounding box (matches
        # the merge path in `mirror`). Plain cuts don't reach here — origin left untouched.
        if center_pivot and symmetrized:
            from blendertk.xform_utils._xform_utils import XformUtils

            XformUtils.center_pivot(symmetrized, mode="object")
    ...
    def wedge(objects, angle=90.0, divisions=4):
        ...
        **Edit-mode workflow** (mode-aware, like ``crease_edges``): select the face(s) plus one
        ...
        for o in _EditUtilsInternal._meshes(objects):
            ...
            if hinge is None:  # fall back to any selected edge bounding a selected face
                hinge = next(
                    (
                        e
                        for e in bm.edges
                        if e.select and any(f.select for f in e.link_faces)
                    ),
                    None,
                )
            ...
            face_center = sum((f.calc_center_median() for f in faces), Vector()) / len(
                faces
            )
    ...
    def snap_to_grid(objects=None, grid_size=1.0, axes="xyz"):
        ...
        objs = [
            o
            for o in (objects if objects is not None else CoreUtils.selected_objects())
            or []
            if o
        ]
    ...
    @staticmethod
    def snap_to_surface(
        source_meshes, target, offset=0.0, threshold=None, invert=False
    ):
        ...
        sources = [
            o
            for o in ptk.make_iterable(source_meshes)
        ...
        tinv = tmw.inverted()
        tnorm = (
            tmw.to_3x3().inverted().transposed()
        )  # normal-transform matrix (handles non-uniform scale)
    ...
    def get_similar_mesh(
        ...
        *Select Similar*). Each enabled metric (vertex / edge / face / triangle / shell / uvcoord / area /
        ...
        refs = _EditUtilsInternal._meshes(
    ...
    def combine_objects(
        ...
        *,
        ...
        groups = {}
        for o in objs:
            groups.setdefault(_EditUtilsInternal._material_key(o), []).append(o)
        ...
        if cluster_by_distance:
            ...
            for members in groups.values():
                if len(members) < 2:
                    ...
                    continue
                centers = [_EditUtilsInternal._world_bbox_center(o) for o in members]
        ...
        return [
            _EditUtilsInternal._join_copies(members, members[0].name)
    ...
    def detach_components(
        *, duplicate=False, separate=True, separate_each=False, center_pivot=True
    ):
        ...
        for o in _EditUtilsInternal._meshes(objects):
            ...
            for face in bm.faces:
                key = tuple(
                    sorted(
                        (
                            round(v.co.x, round_ndigits),
                            round(v.co.y, round_ndigits),
    ...
    def get_overlapping_duplicates(
        objects=None, retain=None, select=False, delete=False, round_ndigits=5
    ):
        ...
        pool = (
            _EditUtilsInternal._meshes(objects)
            if objects is not None
            else [o for o in bpy.data.objects if o.type == "MESH"]
        )
        retain_set = (
            set(_EditUtilsInternal._meshes(retain)) if retain is not None else set()
        )
        pool = list(
            dict.fromkeys([*pool, *retain_set])
        )  # ensure retained are grouped, preserve order

        def _r(p):
            return (
                round(p.x, round_ndigits),
                round(p.y, round_ndigits),
                round(p.z, round_ndigits),
            )
        ...
        def _fingerprint(o):
            ...
            xs, ys, zs = (
                ...
                [p.z for p in corners],
            )
            bbox = (
                round(min(xs), round_ndigits),
                round(min(ys), round_ndigits),
                round(min(zs), round_ndigits),
                round(max(xs), round_ndigits),
                round(max(ys), round_ndigits),
                round(max(zs), round_ndigits),
            )
    ...
    def loft(objects=None, *, close=False, reverse_normals=False, section_spans=1):
        ...
        rings = [_EditUtilsInternal._resample_polyline(p, n) for p in profiles]

        if (
            section_spans and section_spans > 1
        ):  # interpolate extra rings between profiles
            dense = []
