...
Houses :meth:`TransformDiagnostics.fix_non_orthogonal_axes` — non-orthogonal (sheared) object
axes break FBX export.
...
class TransformDiagnostics(_TransformDiagnosticsInternal):
    ...
    def fix_non_orthogonal_axes(objects=None, dry_run=False, tolerance=1e-5):
        ...
        flagged = [
            o
            for o in pool
            if _TransformDiagnosticsInternal._has_shear(
                o.matrix_world.to_3x3(), tolerance
            )
        ]
        if dry_run:
            for o in flagged:
