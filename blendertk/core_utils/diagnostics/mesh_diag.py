...
Houses :meth:`MeshDiagnostics.find_problem_geometry` (re-homed here from ``edit_utils`` to mirror
mayatk, where mesh problem-detection lives in ``Diagnostics`` not ``EditUtils``). The shared
bmesh-iteration helper ``_bmesh_each`` lives in ``edit_utils._edit_utils``; the ``@_object_mode``
guard is canonical in the sibling ``core_utils._core_utils`` — both imported here (the same way
mayatk's diagnostics modules import ``XformUtils`` / ``NodeUtils``).
"""
...
class _MeshDiagnosticsInternal(object):
    """Internal helpers for MeshDiagnostics."""
...
class MeshDiagnostics(_MeshDiagnosticsInternal):
    ...
    def find_problem_geometry(
        ...
        *,
        ...
        area_tolerance=1e-6,
        edge_length_tolerance=1e-6,
        uv_area_tolerance=1e-6,
        select=True,
    ):
        ...
        def _detect(bm):
            ...
            if concave:
                f = [
                    ...
                    if not _MeshDiagnosticsInternal._is_convex(face)
                ]
            ...
            if nonplanar:
                f = [
                    face
                    for face in bm.faces
                    if not _MeshDiagnosticsInternal._is_planar(face, planar_tolerance)
                ]
            ...
            if zero_length_edges:
                e = [
                    ...
                    if edge.calc_length() <= edge_length_tolerance
                ]
            ...
            if zero_uv_area:
                ...
                if uv_layer is not None:
                    f = [
                        face
                        for face in bm.faces
                        if _MeshDiagnosticsInternal._uv_face_area(face, uv_layer)
                        <= uv_area_tolerance
                    ]
        ...
        EditUtils._bmesh_each(objects, _detect)
        counts["_mode"] = mode
        return counts
