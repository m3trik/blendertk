...
class _CamUtilsInternal(object):
    """Internal helpers for CamUtils."""
    ...
    def _resolve_cameras(camera):
        ...
        return [
            o for o in ptk.make_iterable(camera) if getattr(o, "type", None) == "CAMERA"
        ]
    ...
    def _orbit_rotation(view_rotation, dx, dy, sens=_ORBIT_SENS):
        ...
        elevation = Quaternion(
            right, dy * sens
        )  # +dy: drag up tilts the view up (Maya tumble feel)
    ...
    @staticmethod
    def _track_location(
        view_location, view_rotation, view_distance, dx, dy, sens=_TRACK_SENS
    ):
    ...
    def _apply_view_nav(rv3d, mode, dx, dy):
        ...
        if mode == "ORBIT":
            rv3d.view_rotation = _CamUtilsInternal._orbit_rotation(
                rv3d.view_rotation, dx, dy
            )
        elif mode == "ROLL":
            rv3d.view_rotation = _CamUtilsInternal._roll_rotation(
                rv3d.view_rotation, dx
            )
        elif mode == "DOLLY":
            rv3d.view_distance = _CamUtilsInternal._dolly_distance(
                rv3d.view_distance, dy
            )
        elif mode == "TRACK":
            rv3d.view_location = _CamUtilsInternal._track_location(
    ...
    def _view_nav_modal(op, context, event):
        ...
        if et == "MOUSEMOVE":
            if op._dragging:
                ...
                _CamUtilsInternal._apply_view_nav(
                    op._rv3d, op.mode, x - op._last[0], y - op._last[1]
                )
                op._last = (x, y)
                if context.area is not None:
                    context.area.tag_redraw()
        ...
        if et == "ESC" and ev == "PRESS":
            ...
            op._rv3d.view_rotation, op._rv3d.view_location, op._rv3d.view_distance = (
                rot,
                loc,
                dist,
            )
            return _CamUtilsInternal._view_nav_finish(op, context, {"CANCELLED"})
        if et in {"RIGHTMOUSE", "RET", "NUMPAD_ENTER"} and ev == "PRESS":
            return _CamUtilsInternal._view_nav_finish(op, context, {"FINISHED"})
        return {"RUNNING_MODAL"}
    ...
    def _ensure_view_nav_operator():
        ...
        class BTK_OT_view_nav(bpy.types.Operator):
            ...
            mode: bpy.props.EnumProperty(
                ...
                items=[
                    (m, m.title(), m.title())
                    for m in ("ORBIT", "DOLLY", "TRACK", "ROLL")
                ],
                default="ORBIT",
            )

            def invoke(self, context, event):
                return _CamUtilsInternal._view_nav_invoke(self, context, event)

            def modal(self, context, event):
                return _CamUtilsInternal._view_nav_modal(self, context, event)
...
class CamUtils(_CamUtilsInternal):
    ...
    def adjust_camera_clipping(camera=None, near_clip=None, far_clip=None):
        ...
        """
        cams = _CamUtilsInternal._resolve_cameras(camera)
        if not cams:
            return
        needs_auto = near_clip == "auto" or far_clip == "auto"
        corners = _CamUtilsInternal._scene_bbox_corners() if needs_auto else None
        for cam in cams:
            data = cam.data
            max_dist = 0.0
            ...
            if near_clip is not None:
                val = _CamUtilsInternal._resolve_clip(near_clip, max_dist, near=True)
                if val is not None:
                    data.clip_start = val
    ...
    def navigate_view(mode="ORBIT"):
        ...
        _CamUtilsInternal._ensure_view_nav_operator()
