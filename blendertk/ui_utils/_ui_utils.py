...
class _UiUtilsInternal(object):
    """Internal helpers for UiUtils."""
    ...
    def _fullscreen_toggle(win, hide_panels, area=None):
        ...
        if area is None:
            area = _UiUtilsInternal._biggest_area(win)
...
class UiUtils(_UiUtilsInternal):
    ...
    def open_editor(editor, properties_context=None):
        ...
        if ui_type == "PREFERENCES":
            ...
            return next(
                (
                    w
                    for w in reversed(list(bpy.context.window_manager.windows))
        ...
        try:
            ...
            RuntimeError
        ):  # poll refused (no window at all, fullscreen screen, --background)
            return None
        new = [
            w
        ...
        try:
            ...
            _UiUtilsInternal._close_window(
                window
            )  # we opened it; don't strand a duplicate viewport
            return None
    ...
    def find_editor(editor, window=None):
        ...
        """
        ui_type = EDITOR_TYPES.get(editor, editor)
        win = window or UiUtils.main_window()
        if win is None:
            return []
    ...
    def close_editor(editor, window=None):
        ...
        for _ in range(16):  # safety cap; normally exactly one docked strip
            matches = UiUtils.find_editor(editor, window=window)
            if not matches:
                break
            win, area = matches[0]
            if not UiUtils.close_area(win, area):
                break
            closed += 1
        return closed
    ...
    def dock_editor(editor, edge_size=70, window=None):
        ...
        try:
            with bpy.context.temp_override(
                ...
                screen=win.screen,
                area=target,
                region=_UiUtilsInternal._window_region(target),
            ):
        ...
        halves = [a for a in win.screen.areas if a.as_pointer() not in before] + [
            target
        ]
    ...
    def toggle_editor(editor, edge_size=70, window=None):
        ...
        *chrome* with a single ``mel.eval("ToggleTimeSlider")``; Blender editors are tiled *areas*
        ...
        """
        win = window or UiUtils.main_window()
        if UiUtils.find_editor(editor, window=win):
            UiUtils.close_editor(editor, window=win)
            return False
        if UiUtils.dock_editor(editor, edge_size=edge_size, window=win) is None:
            ...
            return UiUtils.open_editor(editor) is not None
        return True
    ...
    def toggle_fullscreen_area(editor=None, hide_panels=True, window=None):
        ...
        if scr.show_fullscreen:
            guess = _UiUtilsInternal._window_bars_hidden(win)
            for hide in (guess, not guess):
                try:
                    _UiUtilsInternal._fullscreen_toggle(win, hide_panels=hide)
                except (RuntimeError, TypeError):
                    pass
        ...
        try:
            res = _UiUtilsInternal._fullscreen_toggle(win, hide_panels, area=target)
        except (RuntimeError, TypeError):
            return False
        return res == {"FINISHED"}
    ...
    def toggle_window_bars(visible=None, window=None):
        ...
        if bpy.context.window is None:
            return None
        hidden = _UiUtilsInternal._window_bars_hidden(win)
        ...
        saved = _UiUtilsInternal._region_flags(_UiUtilsInternal._biggest_area(win))
        try:
            _UiUtilsInternal._fullscreen_toggle(
                win, hide_panels=True
            )  # enter SCREENFULL (hides the bars)
            if visible:
                _UiUtilsInternal._fullscreen_toggle(
                    win, hide_panels=True
                )  # state-matched exit: bars come back
            else:
            ...
            if win.screen.show_fullscreen:  # never strand the window in fullscreen mode
                try:
                    _UiUtilsInternal._fullscreen_toggle(win, hide_panels=True)
                except (RuntimeError, TypeError):
                    pass
            if not win.screen.show_fullscreen:
                _UiUtilsInternal._apply_region_flags(
                    _UiUtilsInternal._biggest_area(win), saved
                )
        return not _UiUtilsInternal._window_bars_hidden(win)
    ...
    def dispatch_log_link(url, logger=None) -> bool:
        ...
        obj = next(
            (o for o in bpy.data.objects if o.name == node and o.library is None), None
        ) or bpy.data.objects.get(node)
        if obj is None:
            if logger:
