...
Subsequent exports compare against the manifest to detect accidental structural changes
(missing/extra nodes, reparenting).
...
class HierarchySidecar:
    ...
    Sidecar files:
        - ``.{stem}.hierarchy.json`` — manifest of hierarchy paths.
        - ``.{stem}.hierarchy_diff.txt`` — human-readable diff report.

    When ``base_stem=True`` is passed to the path helpers, a trailing ``_v\\d+`` suffix is
    stripped so that all versioned exports of a series share a single sidecar (the baseline rolls
    forward to the most recent export). Used by the SceneExporter ``version`` task.
    """
    ...
    def manifest_path_for(cls, export_path: str, *, base_stem: bool = False) -> str:
        ...
        Parameters:
            ...
            base_stem: If True, strip the version suffix so all versions of a series share one
                manifest. Opt-in: callers without an active versioning context should leave this
                False to preserve per-file sidecar behavior.
        """
    ...
    def find_legacy_manifest(cls, export_path: str) -> Optional[str]:
        ...
        Scans the export directory for ``.{base}_v<N>.hierarchy.json`` files and returns the one
        with the highest version number (compared as integers, so unpadded ``_v10`` ranks above
        ``_v2``). Returns None if no legacy sidecars exist.
        """
        directory = os.path.dirname(export_path)
    ...
    def ensure_base_name(cls, export_path: str) -> Optional[str]:
        ...
        Called by callers that opt into ``base_stem=True``. If a base-stem manifest already
        exists, returns its path unchanged. Otherwise, if a legacy ``_v\\d+`` manifest exists,
        renames it to the base-stem path and returns the new path. Returns None if nothing was
        found. Idempotent: callers can invoke this on every check without harm.
        """
    ...
    def rename(cls, old_export_path: str, new_export_path: str) -> list:
        ...
        Moves the ``.hierarchy.json`` manifest and ``.hierarchy_diff.txt`` report (if they exist)
        so that subsequent hierarchy checks find the baseline data under the new export name.
    ...
    def expand_to_descendants(objects) -> list:
        ...
        Mirror of mayatk's version, which used ``cmds.listRelatives(allDescendents=True)`` on DAG
    ...
    def detect_reparenting(missing: list, extra: list) -> list:
        ...
        When a subtree is moved under a new parent every original path appears in *missing* and
        the same paths prefixed with the new parent appear in *extra*. This method finds those
        patterns and returns a list of ``(root_missing, new_parent, count)`` tuples describing
        each reparenting. Unmatched paths are ignored.
        """
    ...
    def write_manifest(cls, export_path: str, paths, *, base_stem: bool = False) -> Optional[str]:
        ...
        the last-known-good baseline is always available.
    ...
    def write_diff_report(
        ...
        *,
        ...
        The report contains a summary with top-level rollups followed by a full path listing.
        Reparenting patterns are called out at the top.
