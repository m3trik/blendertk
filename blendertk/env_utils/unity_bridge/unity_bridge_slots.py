...
and the ``b000`` send action.

The required 'Output Dir' row is repurposed as the **Unity Project** path (the folder containing
``Assets/``); there's no .blend-dir fallback (a saved-scene dir isn't a Unity project), so
:meth:`default_output_dir` returns "". Delivery is a single target: export the selection and
copy the FBX into the project's ``Assets/`` (optionally launching the chosen Editor). The
project create / launch engine is shared (``unitytk.UnityLauncher`` / ``UnityFinder``, the SAME
classes mayatk's Unity bridge uses); this file only wires the Qt glue.
...
_PRESETS_ROOT = Path("blendertk/unity_bridge")


class UnityBridgeSlots(BlenderBridgeSlotsBase):
    """Slots wired to ``unity_bridge.ui`` via :class:`BlenderBridgeSlotsBase`.
    ...
    # Single delivery target: export the selection and copy the FBX into the Unity project's
    # Assets/ (optionally launching the chosen Editor). Matches the shared
    ...
    # The project actions (Set / Open / New) live on the Unity Project field's option menu
    # (see _configure_output_dir_options), co-located with the field they act on.
    HEADER_MENU_ITEMS = (
    ...
    def template_dir(self) -> Path:
        # No script templates (copy-to-assets renders nothing); the package dir is a harmless
        # stand-in for the (no-op) per-template description lookup.
        return _PKG_DIR
    ...
    def default_output_dir(self) -> str:
        # No .blend-dir fallback -- a saved-scene dir isn't a Unity project.
        return ""

    def _relevant_param_keys(self):
        # Copy-to-assets is template-free, so every parameter stays visible.
        return None

    def _configure_output_dir_options(self, edit) -> None:
        """Unity Project field: recent-history button + an option menu of project actions
        (Set Project / Open / New), co-located with the field they act on."""
    ...
    def _resolve_scope_objects(self, scope: str):
        """Objects to export for the chosen ``SCOPE`` param. ``"selected"`` is the default
        (and the fallback for any unknown value)."""
        import bpy
        import blendertk as btk

        if scope == "all":
            # The current SCENE's objects, not every mesh datablock in the .blend (bpy.data.objects
            # would also sweep in unlinked/orphaned objects and objects belonging to other scenes).
...
if __name__ == "__main__":
    ...
    ui = BlenderUiHandler.instance().get("unity_bridge", reload=True)
