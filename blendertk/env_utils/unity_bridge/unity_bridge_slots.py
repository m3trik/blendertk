...
Subclass of uitk's :class:`BridgeSlotsBase` (mirror of mayatk's ``UnityBridgeSlots``). The panel
machinery (parameter widgets, user presets, log routing) lives upstream; this file owns the
Unity-specific bits: the bridge factory, the delivery-mode listing, the relabeled 'Unity Project'
row, the header menu, and the ``b000`` send action.

The required 'Output Dir' row is repurposed as the **Unity Project** path (the folder containing
``Assets/``); there's no fallback. The delivery is template-free, so every parameter is always
visible (:meth:`_relevant_param_keys` returns ``None``). The Qt-only imports live here, not in the
engine, so the engine surface still resolves under headless ``blender --background``.
"""
...
class UnityBridgeSlots(BridgeSlotsBase):
    """Slots wired to ``unity_bridge.ui`` via :class:`BridgeSlotsBase`.

    Discovered by ``BlenderUiHandler`` (``marking_menu.show("unity_bridge")``).
    """
