...
*Export* knobs drive the Blender-side FBX export (read by
:class:`blendertk.env_utils.handoff_export.BlenderExportMixin`); *Unity* knobs drive
the copy-to-Assets delivery (read by :class:`unitytk.CopyToAssetsDeliverer`). Mirrors
:mod:`mayatk.env_utils.unity_bridge.parameters` key-for-key so the two Unity panels
stay 1:1 (Blender's unit-scale conversion stays at the export mixin's default --
both Blender and Unity are metres, so no user knob is needed).
"""
...
PARAMS: "dict[str, AttributeSpec]" = {
    "INCLUDE_MATERIALS": AttributeSpec(
        ...
        tooltip=(
            ...
            "the default shader (materials stripped Blender-side); geometry only."
        ),
    ),
