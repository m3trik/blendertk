...
class HierarchyBaseline:
    ...
    def migrate_from_sidecar(cls, export_dir: str) -> int:
        """Adopt any on-disk baselines in *export_dir* into the file, once.

        Reads every ``.{stem}.scene_data.json`` in the folder -- and the v1
        ``.hierarchy.json`` spelling -- and merges its
        ``hierarchy.paths`` into the file's record, so upgrading does not throw
        ...
        Runs only when the file has no record yet -- a merge of stale per-name
