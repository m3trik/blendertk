...
``env_utils.hierarchy_sync.scene_data_sidecar``.
...
- ``hierarchy`` — change-detection baseline: sorted hierarchy paths from the
  last successful export plus a SHA-256 hash.  Subsequent exports compare
  against it to detect accidental structural changes (missing/extra nodes,
  reparenting).  :meth:`SceneDataSidecar.compare` reads ONLY
  ``paths``/``hash``, and the hash covers only the paths — so neither
  metadata churn nor a recorded ``last_diff`` can false-positive the
  hierarchy check.  ``last_diff`` appears only when a hierarchy check
  flagged a structural change on the export that wrote this manifest (the
  exporter-side check isn't ported to blendertk yet, so today no blendertk
  writer records one): it records that accepted diff, and the next clean
  export drops it.
...
window is gone.  Every successful write sweeps the superseded companion
files, so delivery folders self-clean as assets re-export.  On Windows the
manifest additionally gets ``FILE_ATTRIBUTE_HIDDEN`` (the dot-prefix
convention hides nothing there); ``os.replace`` hands the result the tmp
file's attributes, so the flag is re-applied after every write.
...
Almost entirely DCC-agnostic (pure file I/O + set diffing, ported verbatim)
— the one exception is :meth:`expand_to_descendants`, which walks a live
object's subtree via ``children_recursive`` instead of Maya's
``cmds.listRelatives(allDescendents=True)``.  Blender paths need no
namespace cleaning (see ``_hierarchy_sync.build_path``), so
:meth:`build_clean_path_set` is a plain dedup rather than a
namespace-stripping pass.
"""
...
class SceneDataSidecar:
    ...
    When ``base_stem=True`` is passed to the path helpers, a trailing ``_v\\d+``
    suffix is stripped so that all versioned exports of a series share a single
    sidecar (the record rolls forward to the most recent export).  Used by the
    SceneExporter ``version`` task.
    """
    ...
    def manifest_path_for(cls, export_path: str, *, base_stem: bool = False) -> str:
        ...
        Parameters:
            ...
            base_stem: If True, strip the version suffix so all versions of a
                ...
                versioning context should leave this False to preserve per-file
                sidecar behavior.
        """
    ...
    def _safe_replace(src: str, dst: str) -> bool:
        ...
        a *successful* FBX write as a failed export (measured on the mayatk
        twin).  The caller keeps working with whichever file still exists;
        returns True when the rename actually happened.
        """
        try:
            os.replace(src, dst)
            return True
    ...
    def ensure_base_name(cls, export_path: str) -> Optional[str]:
        ...
        new path. Returns None if nothing was found. Idempotent: callers can
        invoke this on every check without harm.
        """
        ...
        if not cls._safe_replace(legacy, new_path):
            ...
            # found (a fresh one is written after export) rather than raising
            # into the exporter.
            return None
    ...
    def detect_reparenting(missing: list, extra: list) -> list:
        ...
        When a subtree is moved under a new parent every original path appears in
        *missing* and the same paths prefixed with the new parent appear in *extra*.
        This method finds those patterns and returns a list of
        ``(root_missing, new_parent, count)`` tuples describing each reparenting.
        Unmatched paths are ignored.
        """
    ...
    def write_manifest(
        ...
        *,
        ...
        Parameters:
            ...
            paths: Iterable of hierarchy path strings.
            data: Decoded ``data_export`` channel snapshot to record
                (``DataNodes.dump(decode=True)`` shape).  Omitted from the
                manifest when empty.
            last_diff: A hierarchy check's mismatch record for THIS export
    ...
    def read_manifest(
        ...
        Returns:
            A set of hierarchy path strings, or ``None`` if neither the
    ...
    def format_diff_report(
        ...
        The report contains a summary with top-level rollups followed by a
        full path listing.  Reparenting patterns are called out at the top.
        Callers decide where it goes — a report belongs in a temp artifact
        linked from the log, never in the export folder (the structured
        record lives in the manifest's ``hierarchy.last_diff``).
    ...
    def build_full_path_set(cls, objects) -> set:
        """Expand *objects* to descendants, then dedup."""
        return cls.build_clean_path_set(cls.expand_to_descendants(objects))

    @classmethod
    def compare(
        cls, export_path: str, current_paths: set, *, base_stem: bool = False
    ) -> Tuple[bool, list, list]:
        """Compare *current_paths* against the stored hierarchy baseline.

        Uses the stored hash for a fast-path equality check before falling
        back to a full set diff.  When the manifest is missing or unreadable
        a surviving v2-era ``.prev`` backup is used as the baseline; with
        neither present the check passes (no baseline yet).  Reads all
        manifest formats; the data and ``last_diff`` sections are ignored.

        Returns:
            ``(match, missing, extra)`` where *match* is ``True`` when the
