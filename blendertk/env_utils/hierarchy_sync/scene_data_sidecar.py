...
- ``hierarchy`` — change-detection baseline: sorted hierarchy paths from the
  ...
  reparenting).  :meth:`SceneDataSidecar.compare` reads ONLY this section,
  and the hash covers only the paths — so metadata churn can never
  false-positive the hierarchy check.
...
class SceneDataSidecar:
    ...
    sidecar (the record rolls forward to the most recent export, with ``.prev``
    holding the one before it). Used by the SceneExporter ``version`` task.
    """
    ...
    def rename(cls, old_export_path: str, new_export_path: str) -> list:
        ...
        report (if they exist) so that subsequent hierarchy checks find the
        baseline data under the new export name.  Any v1-named sidecars are
        promoted to the current naming first, so one pass covers them.
    ...
    def write_manifest(
        ...
        *,
        ...
        Before overwriting, the existing manifest (if any) is preserved as a
        ``.prev`` file — so the previous export's full record stays available —
        unless neither the hierarchy nor the data section changed.
        ...
        # Write the new manifest to a temp file FIRST — moving the old one to a
        # .prev before a failed write would leave no manifest at all (compare()
    ...
    def _load_manifest(cls, manifest_path: str) -> Optional[dict]:
        ...
        Accidental deletion or a corrupt write usually leaves the ``.prev`` backup
        intact — comparing against the last-known-good baseline beats silently
        passing. Returns None when neither file is readable (no baseline yet).
        """
    ...
    def read_manifest(
        ...
        Falls back to the ``.prev`` backup when the manifest itself is missing
        or unreadable.  Reads both format v2 and flat v1 manifests.
    ...
    def write_diff_report(
        ...
        *,
        ...
        The report contains a summary with top-level rollups followed by a full
        path listing.  Reparenting patterns are called out at the top.
    ...
    def compare(
        ...
        Uses the stored hash for a fast-path equality check before falling back
        to a full set diff.  When the manifest is missing or unreadable its
        ``.prev`` backup is used as the baseline; with neither present the check
        passes (no baseline yet).  Reads both format v2 and flat v1 manifests;
        the data section is ignored.
