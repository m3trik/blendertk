...
class KeyStashSlots(ptk.LoggingMixin):
    ...
    wiring between the two panels.  ``bpy`` and Qt are imported lazily so the
    module surface resolves headless.
    """
    ...
    def header_init(self, widget) -> None:
        ...
        widget.set_help_text(
            self.sb.tooltip.fmt(
                ...
                steps=[
                    "Pick a <b>Source</b>: the selected keys, the timeline's "
                    "preview range, or the scene frame range (the last two act "
                    ...
                    "objects through a temporary override, without retrieving "
                    "it; <b>In Context</b> keeps the scene animation outside "
                    "the clip's range.",
                    "<b>Retrieve</b> puts the keys back (on their original "
                    "frames, or starting at the current frame) and forgets the "
                    "clip. <b>Drop</b> deletes them for good.",
                ],
                notes=[
                    "Double-click a clip to select its objects.",
                    "Refresh (header) re-reads the file: clips whose stash "
                    "actions were deleted or undone away are pruned.",
                ],
            )
        )
    ...
    def refresh_from_scene(self) -> None:
        """Header refresh: prune clips whose stash actions are gone, then repaint."""
        gone = self.store.reconcile()
        self.refresh()
        if gone:
            self._footer(
                f"Pruned {len(gone)} clip(s) whose keys are gone from the file.",
                "warning",
            )
        else:
            self._footer("Clip list is in sync with the file.")
    ...
    def _select_clip_objects(self, item, _column: int = 0) -> None:
        ...
        if clip is None:
            return
        present = [o for o in map(bpy.data.objects.get, clip.objects) if o is not None]
        if not present:
