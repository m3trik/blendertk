...
class _BakeSessionStoreInternal(object):
    ...
    def _restore_blend_shape_driver(entry: dict, warnings: List[str]) -> Optional[str]:
        ...
        key_block = (
            shape_keys.key_blocks.get(entry.get("key_block")) if shape_keys else None
        )
        ...
        for var_entry in entry.get("variables", []):
            ...
            target_obj = BakeSessionStore.resolve_ref(var_entry.get("target"))
            ...
            RigUtils.add_prop_var(
                fcurve, name, target_obj, var_entry.get("data_path", "")
            )
    ...
    def _restore_blend_shape_action(entry: dict, warnings: List[str]) -> Optional[str]:
        ...
        for key_entry in entry.get("keys", []):
            ...
            k.handle_right_type = key_entry.get(
...
class BakeSessionStore(_BakeSessionStoreInternal):
    ...
    def resolve_ref(ref: Optional[Dict[str, str]]):
        ...
        if not name:
            return None
        collection = (
            bpy.data.actions if ref.get("kind") == "action" else bpy.data.objects
        )
    ...
    def resolve_constraint(ref: Optional[Dict[str, Any]]):
        ...
        if not ref:
            return None
        obj = BakeSessionStore.resolve_ref(ref.get("object"))
        if obj is None:
            return None
        bone = ref.get("bone")
        try:
        ...
        for var in drv.variables:
            ...
            if var.type in _REBUILDABLE_VAR_TYPES:
                ...
                if target.id_type == "OBJECT" and target.id is not None:
                    entry["target"] = BakeSessionStore.node_ref(target.id)
    ...
    def restore_session(session: dict) -> "RestoreResult":
        ...
        for entry in session.get("baked_objects", []):
            ...
            if obj is None:
                result.warnings.append(
                    f"Baked object '{object_ref.get('name')}' not found."
                )
                continue
        ...
        for entry in session.get("muted_constraints", []):
            ...
            constraint = BakeSessionStore.resolve_constraint(ref)
        ...
        for entry in session.get("muted_drivers", []):
            ref = entry.get("ref") or {}
            fcurve = BakeSessionStore.resolve_driver(ref)
            if fcurve is None:
                result.warnings.append(
        ...
        for entry in session.get("blend_shape_drivers", []):
            restored = _BakeSessionStoreInternal._restore_blend_shape_driver(
        ...
        for entry in session.get("blend_shape_actions", []):
            restored = _BakeSessionStoreInternal._restore_blend_shape_action(
                entry, result.warnings
            )
            if restored:
                result.blend_shapes_restored.append(restored)
