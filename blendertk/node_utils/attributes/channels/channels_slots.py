...
class ChannelsSlots:
    ...
    def _autofit_window(self, widget):
        ...
        All the sizing — the screen-fraction cap that hands overflow to the table's
        scrollbar, and the deferral that lets ``ResizeToContents`` columns settle
        before they're measured — lives in uitk's
        :meth:`TableWidget.fit_window_to_contents`, so this panel and its mayatk twin
        can't drift apart.
        """
        chk = getattr(self, "_chk_auto_fit", None)
    ...
    def _wire_table_signals(self, widget):
        ...
        # Unlike the signals above these carry no handlers but ours, so the stale binding is
        # dropped by tracked reference instead of a blanket ``disconnect()``: PySide warns
        # ("Failed to disconnect (None) from signal ...") whenever a signal has nothing
        ...
        for name, slot in (
            ...
            old = conns.get(name)
            # A Connection goes falsy once it breaks (receiver collected on a reload), and the
            # STATIC QObject.disconnect is the API that takes one — the signal-instance form
            # expects a *slot* and only warns when handed a Connection, which no ``except``
            # can swallow.
            if old:
                try:
                    self.sb.QtCore.QObject.disconnect(old)
                except (RuntimeError, TypeError):
