...
|:---|:---|:---:|:---|
| `core_utils` | `CoreUtils` | ✅ | `ScriptJobManager` (`bpy.app.handlers`), `Preview`, `diagnostics/` (`mesh_diag`, `transform_diag`; mayatk's animation/scene/uv diag not ported until a slot needs them). Maya's `Components` folds into `edit_utils` (no component-string model). |
...
| `node_utils` | `NodeUtils` | ✅ | `DataNodes`; `attributes/channels/` **Channels** tool in both (Blender maps channels onto transform channels + ID props; no `_attributes.py` mirror — Maya's enum/mute/channel-box helpers have no analogue). |
| `cam_utils` | `CamUtils` | ✅ | |
...
| `mat_utils` | `MatUtils` | ✅ | `mat_updater`, `texture_path_editor`, `shader_templates`, `game_shader`, `render_opacity/`, `image_to_plane/`, `texture_baker.py` (`TextureBaker`, composed by `LightmapBaker`). mayatk's `substance_bridge`/`marmoset_bridge` (live RPC) + `ShaderAttributeMap` cluster are not mirrored — single Principled BSDF + native ops cover it (ledgered). |
| `anim_utils` | `AnimUtils` | ✅ | `scale_keys.py`, `stagger_keys.py` (thin — no segment/overlap/speed machinery, ledgered). mayatk's shot/blendshape/audio subtrees are open ports (see plan). |
| `edit_utils` | `EditUtils` | ✅ | `Macros`, `curtain` (+`CurtainRig`, engine-level like Maya), `mirror`, `cut_on_axis`, `duplicate_linear/radial/grid`, `bevel`, `bridge`, `snap`, `naming/`, `dynamic_pipe` (handler-launched only, like Maya). Object↔Edit round-trips via the shared `_edit_mesh_each`. |
