...
- [`anim_utils/key_stash/_key_stash.py`](#anim_utils--key_stash--_key_stash) — Key Stash — park keyframes outside the working animation, retrieve later (Blender).
- [`anim_utils/key_stash/key_stash_slots.py`](#anim_utils--key_stash--key_stash_slots) — Slots for the Key Stash panel (key_stash.ui) — mirror of mayatk's ``KeyStashSlots``.
...
- [`anim_utils/segment_keys.py`](#anim_utils--segment_keys) — Animation-segment collection over Blender fcurves (mirror of ``mtk.SegmentKeys``).
- [`anim_utils/shots/_detection.py`](#anim_utils--shots--_detection) — Shot-region detection — Blender scene acquisition over the pure engine math.
...
- [`audio_utils/segments.py`](#audio_utils--segments) — Consumer-facing audio-segment discovery for the sequencer + manifest (Blender).
...
- [`env_utils/handoff_export.py`](#env_utils--handoff_export) — Blender-side selection + export hooks shared by the hand-off bridge engines.
...
- [`node_utils/data_nodes.py`](#node_utils--data_nodes) — The Blender scene store -- mirror of mayatk's ``node_utils.data_nodes``.
...
- [`ui_utils/node_icons.py`](#ui_utils--node_icons) — Resolve per-object-type icons for Blender objects (mirror of ``mtk.NodeIcons``).
...
- [`ui_utils/ui_state.py`](#ui_utils--ui_state) — Persist Blender's per-session UI visibility state across sessions (``btk.UiState``).
...
- [`DEFAULT_INCLUDE`](blendertk/blendertk/__init__.py#L31) — constant
...
- **[`class AnimUtils(_AnimUtilsInternal)`](blendertk/blendertk/anim_utils/_anim_utils.py#L576)** — Namespace mirror (helpers also exposed module-level).
  ...
  - `AnimUtils.key_times(fc)` *(static)* — Key times of *fc* as a plain float list (time order).
  - `AnimUtils.key_interpolations(fc)` *(static)* — Per-key ``interpolation`` enum ints of *fc* (see :meth:`interpolation_value`).
  - `AnimUtils.window_indices(times, lo, hi, inclusive_hi: bool = True)` *(static)* — ``(i0, i1)`` slice bounds of the keys of sorted *times* inside ``[lo, hi]``.
  - `AnimUtils.shift_keys_in_window(fc, lo, hi, delta: float, inclusive_hi: bool = True) -> int` *(static)* — Translate *fc*'s keys (and both handles) inside ``[lo, hi]`` by *delta*.
  - `AnimUtils.remap_keys_in_window(fc, lo, hi, old_start, old_end, new_start, new_end) -> int` *(static)* — Linearly remap *fc*'s keys (and handles) in ``[lo, hi]`` from one span to another.
  - `AnimUtils.step_last_key_in_window(fc, lo, hi) -> bool` *(static)* — Set the LAST key of *fc* inside ``[lo, hi]`` to ``CONSTANT`` interpolation.
  - `AnimUtils.evaluable_override(objects)` *(static)* — Yield with *objects* evaluated by the depsgraph at ANY frame: revealed
  ...
  - `AnimUtils.remove_preview_layer(handle) -> bool` *(static)* — Tear down :meth:`create_preview_layer`'s tracks and restore the active action.
  - `AnimUtils.interpolation_value(name: str) -> int` *(static)* — Int value of a ``Keyframe.interpolation`` enum item (bulk reads return ints).
...
- [`TWEEN_GROUP_NAME`](blendertk/blendertk/anim_utils/blendshape_animator/target.py#L26) — constant
- **[`class Target`](blendertk/blendertk/anim_utils/blendshape_animator/target.py#L31)** — Represents a single tween (in-between) target mesh object.
...
<a id="anim_utils--key_stash--_key_stash"></a>
### `anim_utils/key_stash/_key_stash.py`
...
<a id="anim_utils--key_stash--key_stash_slots"></a>
### `anim_utils/key_stash/key_stash_slots.py`
...
- **[`class BlenderShotStore(ShotStore, _BlenderShotStoreInternal)`](blendertk/blendertk/anim_utils/shots/_shots.py#L464)** — :class:`pythontk.ShotStore` with the scene hooks bound to Blender.
  ...
  - `BlenderShotStore.publish_export_view(self, strategy: Optional[str] = None) -> Optional[str]` — Publish this store's shot records onto the shared ``data_export`` Empty.
  - `BlenderShotStore.export_transfer(cls, spell=None, objects=None) -> Optional[Dict[str, Any]]` *(class)* — The active store as a hand-off ``shots`` section (``None`` when empty).
  - `BlenderShotStore.apply_transfer(cls, section: Dict[str, Any], *, resolve=None, frame_offset: float = 0.0, replace: bool = False, converted=None) -> Optional['BlenderShotStore']` *(class)* — Rebuild the scene's shots from a hand-off ``shots`` section (mirror of
...
- [`FLOAT_ZERO_EPS`](blendertk/blendertk/anim_utils/shots/shot_sequencer/clip_motion.py#L23) — constant
- **[`class ClipMotionMixin(_ClipMotionMixinInternal)`](blendertk/blendertk/anim_utils/shots/shot_sequencer/clip_motion.py#L50)** — Mixin supplying clip move, resize, and batch-move handlers.
...
- [`TIME_SNAP_EPS`](blendertk/blendertk/anim_utils/shots/shot_sequencer/gap_manager.py#L16) — constant
...
- **[`class MarkerManagerMixin(_MarkerManagerMixinInternal)`](blendertk/blendertk/anim_utils/shots/shot_sequencer/marker_manager.py#L33)** — Mixin supplying marker CRUD persistence.
...
- **[`class ShotSequencerController(GapManagerMixin, ClipMotionMixin, ShotNavMixin, MarkerManagerMixin, ptk.LoggingMixin, _ShotSequencerControllerInternal)`](blendertk/blendertk/anim_utils/shots/shot_sequencer/shot_sequencer_slots.py#L65)** — Business logic controller bridging SequencerWidget ↔ ShotSequencer.
  ...
  - `ShotSequencerController.on_key_tangent_dragged(self, clip_id: int, time: float, side: str, dt: float, dv: float) -> None` — DEPRECATED, one release: the single-key form of
...
- **[`class ShotEditDialog`](blendertk/blendertk/anim_utils/shots/shot_sequencer/shot_sequencer_slots.py#L3529)** — Lightweight dialog for creating or editing a shot (plain Qt widgets).
...
- **[`class ShotSequencerSlots(ptk.LoggingMixin)`](blendertk/blendertk/anim_utils/shots/shot_sequencer/shot_sequencer_slots.py#L3610)** — Switchboard slot class — routes UI events to the controller.
...
- **[`class ShotsController(ptk.LoggingMixin)`](blendertk/blendertk/anim_utils/shots/shots_slots.py#L31)** — Business logic for the Shots settings panel.
  ...
  - `ShotsController.on_shot_name_changed(self, text: str) -> None` — Push a name the store accepts;
...
- **[`class ShotsSlots(ptk.LoggingMixin)`](blendertk/blendertk/anim_utils/shots/shots_slots.py#L1195)** — Switchboard slot class — routes UI events to the controller.
...
- **[`class BakeResult`](blendertk/blendertk/anim_utils/smart_bake/_smart_bake.py#L92)** — Result container for ``SmartBake.bake()``.
  - `BakeResult.skip(self, key: str, reason: str) -> None` — Record *key* as skipped, and why -- the one way into ``skipped`` (mirror of
  - `BakeResult.declined(self) -> Dict[str, str]` *(property)* — ``{key: reason}`` for what the bake REFUSED or failed to key --
...
- **[`class SmartBake(_SmartBakeInternal)`](blendertk/blendertk/anim_utils/smart_bake/_smart_bake.py#L349)** — Intelligent bake+restore with automatic detection of what needs baking.
...
- **[`class AudioUtils(ptk.LoggingMixin)`](blendertk/blendertk/audio_utils/_audio_utils.py#L73)** — Scene-wide audio-clip CRUD over Blender's Video Sequence Editor.
  ...
  - `AudioUtils.get_fps(scene=None) -> float` *(static)* — Effective scene framerate (``render.fps / fps_base``), ``24.0`` outside Blender.
  - `AudioUtils.clips_in_range(cls, start: float, end: float, scene=None) -> List[Dict]` *(class)* — Sound strips whose visible span ``[frame_start, frame_end]`` overlaps ``[start, end]``.
  - `AudioUtils.shift_clips_in_range(cls, old_start: float, old_end: float, delta: float, names: Optional[List[str]] = None, scene=None) -> List[str]` *(class)* — Shift every sound strip whose visible start lies in ``[old_start, old_end]`` by *delta*.
  - `AudioUtils.cached_waveform(cls, path: str) -> List[Tuple[float, float]]` *(class)* — Waveform envelope for *path*, computed once per path (mirror of mayatk).
...
- **[`class AudioSegment(_AudioSegmentInternal)`](blendertk/blendertk/audio_utils/segments.py#L55)** — A resolved audio segment for sequencer/manifest consumption.
  - `AudioSegment.is_audio(self) -> bool` *(property)*
  - `AudioSegment.collect_all_segments(scene_start: Optional[float] = None, scene_end: Optional[float] = None, include_waveform: bool = True, scene=None) -> List['AudioSegment']` *(static)* — Return every :class:`AudioSegment` in the scene's sequence editor.
  - `AudioSegment.collect_segments_for_track(track_id: str, include_waveform: bool = True, scene=None) -> List['AudioSegment']` *(static)* — Return the segment for a single strip *track_id* (``[]`` if absent).
...
- **[`class CoreUtils(ptk.CoreUtils, _CoreUtilsInternal)`](blendertk/blendertk/core_utils/_core_utils.py#L336)** — Blender ``CoreUtils`` — extends pythontk's DCC-agnostic ``CoreUtils`` (mirrors
  ...
  - `CoreUtils.user_config_path(filename, base=None)` *(static)* — Absolute path of ``filename`` in Blender's user ``CONFIG`` dir (where Blender keeps
...
- [`ASSEMBLY_TAG_ATTR`](blendertk/blendertk/core_utils/auto_instancer/assembly_reconstructor.py#L42) — constant
- [`CANONICAL_BAKE_PREFIX`](blendertk/blendertk/core_utils/auto_instancer/assembly_reconstructor.py#L48) — constant
...
- **[`class TransformDiagnostics(_TransformDiagnosticsInternal)`](blendertk/blendertk/core_utils/diagnostics/transform_diag.py#L122)** — Transform/shear diagnostics (mirror of mayatk's ``TransformDiagnostics``).
...
- [`COLOR_PROP`](blendertk/blendertk/display_utils/outliner_tint.py#L46) — constant
- **[`class OutlinerTint(_OutlinerTintInternal)`](blendertk/blendertk/display_utils/outliner_tint.py#L267)** — Per-object outliner text colour (Maya ``outlinerColor`` analogue).
...
- **[`class EditUtils(_EditUtilsInternal)`](blendertk/blendertk/edit_utils/_edit_utils.py#L702)** — Namespace mirror of mayatk's ``EditUtils`` (helpers also exposed module-level).
  ...
  - `EditUtils.mirror(objects, axis='x', pivot='object', merge_mode=1, delete_original=False, merge_threshold=0.001, center_pivot=True, axis_frame=None)` *(static)* — Mirror mesh object(s) across an axis plane — mirror of ``mtk.EditUtils.mirror``.
  - `EditUtils.mirror_instance(objects, axis='x', pivot='object', axis_frame=None)` *(static)* — Mirror as **linked duplicates**: each object gets a copy that shares its mesh data,
  - `EditUtils.cut_along_axis(objects, axis='x', pivot='center', amount=1, offset=0.0, spacing=0.0, distribution='linear', weight_bias=0.5, weight_curve=2.0, invert=False, delete=False, mirror=False, merge_threshold=0.0001, center_pivot=True, axis_frame=None)` *(static)* — Cut mesh object(s) along an axis — mirror of ``mtk.EditUtils.cut_along_axis``.
...
- **[`class DisplayMacros(_ViewportMixin)`](blendertk/blendertk/edit_utils/macros.py#L91)**
  ...
  - `DisplayMacros.m_cycle_background(cls)` *(class)* — Cycle the viewport background: theme -> black -> dark/mid/light gray -> white.
...
- **[`class Naming(ptk.HelpMixin, ptk.LoggingMixin)`](blendertk/blendertk/edit_utils/naming/_naming.py#L28)** — Batch find / rename / suffix scene objects (mirror of mayatk's ``Naming``).
  ...
  - `Naming.affix_for(cls, item, overrides=None, modes=None)` *(class)* — The convention's affix rule for *item*'s OWN type -- mirror of mayatk's.
...
- [`COLOR_PRESELECT`](blendertk/blendertk/edit_utils/target_weld.py#L51) — constant
- [`COLOR_SOURCE`](blendertk/blendertk/edit_utils/target_weld.py#L52) — constant
- [`COLOR_LINE`](blendertk/blendertk/edit_utils/target_weld.py#L53) — constant
- [`COLOR_HALO`](blendertk/blendertk/edit_utils/target_weld.py#L54) — constant
- [`COLOR_PROMPT_BG`](blendertk/blendertk/edit_utils/target_weld.py#L55) — constant
- [`COLOR_PROMPT_TEXT`](blendertk/blendertk/edit_utils/target_weld.py#L56) — constant
- [`PICK_RADIUS`](blendertk/blendertk/edit_utils/target_weld.py#L58) — constant
- [`MARKER_SIZE`](blendertk/blendertk/edit_utils/target_weld.py#L59) — constant
- [`HALO_SIZE`](blendertk/blendertk/edit_utils/target_weld.py#L60) — constant
...
- **[`class FbxUtils(_FbxUtilsInternal)`](blendertk/blendertk/env_utils/fbx_utils.py#L590)** — FBX import / export over ``bpy.ops`` (mirror of mayatk's ``FbxUtils`` export surface).
  ...
  - `FbxUtils.publish_authored(cls, records) -> ptk.ExportSnapshot` *(class)* — Commit records a tool already holds -- its AUTHORING-time publish
  - `FbxUtils.stagers(cls, names: Optional[Iterable[str]] = None) -> Dict[str, Tuple[Optional[Callable], Optional[Callable]]]` *(class)* — The stager table for a bracket: the known stagers (*names* narrows
  - `FbxUtils.stage(cls, names: Optional[Iterable[str]] = None)` *(class)* — Run every stager's ``prepare`` now and return the table that ran
  - `FbxUtils.begin_export(cls, ctx: Optional[ptk.ExportContext] = None, only: Optional[Iterable[Any]] = None, stagers: Optional[Iterable[str]] = None) -> Optional[ptk.ExportSnapshot]` *(class)* — Open an export bracket (outermost only): stage the scene, then --
  - `FbxUtils.end_export(cls) -> None` *(class)* — Close an export bracket: run the stagers' finish (outermost only).
  - `FbxUtils.export_prepared(cls, ctx: Optional[ptk.ExportContext] = None, only: Optional[Iterable[Any]] = None, stagers: Optional[Iterable[str]] = None)` *(class)* — Stage the scene (and publish, given a *ctx*) for an export;
  - `FbxUtils.scratch_export(cls)` *(class)* — Bracket for a THROWAWAY FBX write: nothing inside stages or
  - `FbxUtils.enable_export_producer(cls, spec) -> None` *(class)* — Record that *spec*'s producer wants to run before every export.
  - `FbxUtils.disable_export_producer(cls, spec) -> None` *(class)*
  - `FbxUtils.register_export_stager(cls, name: str, prepare: Optional[Callable[[], Any]] = None, finish: Optional[Callable[[], Any]] = None) -> None` *(class)* — Run *prepare* before and *finish* after every bracketed export.
  - `FbxUtils.unregister_export_stager(cls, name: str) -> None` *(class)*
  - `FbxUtils.register_export_preparer(cls, name: str, prepare) -> None` *(class)* **DEPRECATED**
  - `FbxUtils.unregister_export_preparer(cls, name: str) -> None` *(class)* **DEPRECATED**
  - `FbxUtils.run_export_preparers(cls, only: Optional[Iterable[str]] = None) -> None` *(class)* **DEPRECATED**
  - `FbxUtils.bake_range(takes=None)` *(static)* — The ``(start, end)`` frames the next write will actually BAKE.
  ...
  - `FbxUtils.stage_curve_proxy(name: str, parent, fcurve, *markers: str)` *(static)* — Stage one transient Empty whose ``scale.x`` carries *fcurve*, for an FBX write.
  - `FbxUtils.apply_takes_from_node(node=None, attr=None) -> int` *(static)* — Arm the takes the scene declares for the next write.
...
- [`main() -> int`](blendertk/blendertk/env_utils/hierarchy_sync/_fbx_stage_worker.py#L31) — Stage the FBX named on the command line;
...
### `env_utils/hierarchy_sync/hierarchy_baseline.py`
...
- [`SUPPORTED_EXTENSIONS`](blendertk/blendertk/env_utils/maya_bridge/_scene_import.py#L89) — constant
- [`BAKE_SOURCE_EXTENSIONS`](blendertk/blendertk/env_utils/maya_bridge/_scene_import.py#L94) — constant
- [`BAKE_SOURCE_SUFFIX`](blendertk/blendertk/env_utils/maya_bridge/_scene_import.py#L100) — constant
- [`MAYA_GROUP_EMPTY_DISPLAY_SIZE`](blendertk/blendertk/env_utils/maya_bridge/_scene_import.py#L116) — constant
- [`USD_EXTENSIONS`](blendertk/blendertk/env_utils/maya_bridge/_scene_import.py#L120) — constant
- [`FBX_IMPORT_OPTIONS`](blendertk/blendertk/env_utils/maya_bridge/_scene_import.py#L138) — constant
- [`REDUCE_KEYS_DEFAULT`](blendertk/blendertk/env_utils/maya_bridge/_scene_import.py#L149) — constant
- **[`class MayaSceneImport(ptk.LoggingMixin)`](blendertk/blendertk/env_utils/maya_bridge/_scene_import.py#L238)** — Engine: convert a Maya scene to FBX via headless Maya, then import it.
  ...
  - `MayaSceneImport.render_script(self, src_path: str, out_path: str, *, via: str = 'fbx', embed_textures: bool = False, include_animation: bool = True, rig_mode: str = 'auto') -> str` — Render the Maya-side conversion script (exposed for tests/preview).
  ...
  - `MayaSceneImport.import_scene(self, src_path: str, *, via: str = 'fbx', cleanup: bool = True, use_cache: bool = True, timeout: Optional[float] = None, fbx_options: Optional[Dict[str, Any]] = None, rig_mode: str = 'auto', scene_settings: Union[bool, str] = 'auto', reduce_keys: Union[bool, str, None] = REDUCE_KEYS_DEFAULT, shots: bool = True, progress: Optional[Callable[..., Optional[bool]]] = None, **script_opts: Any) -> List[Any]` — Import the Maya scene at *src_path*;
  - `MayaSceneImport.import_payload(self, payload_path: str, *, fbx_options: Optional[Dict[str, Any]] = None, usd_options: Optional[Dict[str, Any]] = None, scene_settings: bool = False, reduce_keys: Union[bool, str, None] = False, shots: bool = True, progress: Optional[Callable[[int, int, str], Optional[bool]]] = None) -> List[Any]` — Import a Maya hand-off payload into the open scene and apply its manifest.
  ...
  - `MayaSceneImport.render_bake_script(self, src_path: str, out_path: str, reduce_keys: Union[bool, str, None] = None) -> str` — Render the Blender-side intermediate->.blend bake script (exposed for
  - `MayaSceneImport.bake(self, src_path: str, out_path: str, *, timeout: Optional[float] = None, reduce_keys: Union[bool, str, None] = None, on_output: Optional[Callable[[Optional[str]], Optional[bool]]] = None) -> Any` — Bake the USD/FBX intermediate *src_path* into the .blend at *out_path*
  - `MayaSceneImport.bake_scene(self, src_path: str, *, via: str = 'fbx', use_cache: bool = True, timeout: Optional[float] = None, rig_mode: str = 'auto', reduce_keys: Union[bool, str, None] = REDUCE_KEYS_DEFAULT, progress: Optional[Callable[..., Optional[bool]]] = None, **script_opts: Any) -> str` — Bake *src_path* to a cached ``.blend`` and return its path — the link path.
...
- **[`class Parameters`](blendertk/blendertk/env_utils/maya_bridge/parameters.py#L131)** — Parameters — module namespace.
...
- [`main()`](blendertk/blendertk/env_utils/maya_bridge/templates/_bake_scene.py#L54)
- [`SRC_FILE`](blendertk/blendertk/env_utils/maya_bridge/templates/_bake_scene.py#L39) — constant
- [`OUT_BLEND`](blendertk/blendertk/env_utils/maya_bridge/templates/_bake_scene.py#L40) — constant
- [`EXTRA_SYS_PATH`](blendertk/blendertk/env_utils/maya_bridge/templates/_bake_scene.py#L41) — constant
- [`REDUCE_KEYS`](blendertk/blendertk/env_utils/maya_bridge/templates/_bake_scene.py#L44) — constant
...
- [`fbx_safe_materials(cmds)`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene.py#L380) — Swap every FBX-hostile shader for an equivalent phong on its shading group.
- [`skinning_methods(cmds)`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene.py#L533) — ``{mesh transform name: skinning method}`` for the manifest's ``skins``
- [`scene_node_types(cmds)`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene.py#L554) — ``{leaf name: "locator" | "group"}`` -- the node-type sidecar section.
- [`scene_settings(cmds)`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene.py#L598) — The scene's time setup -- the manifest's ``scene`` section, the one part of a
- [`write_manifest(entries, visibility, node_types, scene, path, lights=(), skins=None, bones=None, shots=None, rig=None, machinery=None)`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene.py#L625) — The ONE conversion sidecar, consumed by MayaSceneImport: ``materials`` =
- [`scene_lights(cmds)`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene.py#L687) — The manifest's ``lights`` section: every light in the scene, as data.
- [`main()`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene.py#L1235)
- [`shots_section(cmds, spell)`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene.py#L1343) — The scene's shots as the manifest's ``shots`` section, or ``None``.
- [`SRC_PATH`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene.py#L48) — constant
- [`OUT_FBX`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene.py#L49) — constant
- [`EMBED_TEXTURES`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene.py#L50) — constant
- [`INCLUDE_ANIMATION`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene.py#L51) — constant
- [`SMART_BAKE`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene.py#L54) — constant
- [`RIG_MODE`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene.py#L58) — constant
- [`RIG_CAPABILITY`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene.py#L59) — constant
- [`MODERN_SHADER_TYPES`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene.py#L76) — constant
- [`STINGRAY_SHADER_TYPES`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene.py#L79) — constant
- [`STINGRAY_TEX_SLOTS`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene.py#L82) — constant
- [`STINGRAY_SLOT_CHANNELS`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene.py#L96) — constant
...
- [`collect_instance_groups(cmds)`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene_usd.py#L649) — Maya instance sets -> ``[[sanitized prim paths sharing one shape], ...]``.
- [`scene_settings(cmds)`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene_usd.py#L710) — The scene's time setup -- the manifest's ``scene`` section, the one part of a
- [`write_manifest(cmds, materials=None, shading_groups=None, bones=None, shots=None, rig=None, machinery=None)`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene_usd.py#L968) — Sidecar beside the USD carrying what USD itself cannot: instance groups,
- [`main()`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene_usd.py#L1028)
- [`shots_section(cmds, spell)`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene_usd.py#L1092) — The scene's shots as the manifest's ``shots`` section, or ``None``.
...
- [`RIG_MODE`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene_usd.py#L46) — constant
- [`RIG_CAPABILITY`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene_usd.py#L47) — constant
- [`STINGRAY_SHADER_TYPES`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene_usd.py#L51) — constant
- [`STINGRAY_TEX_SLOTS`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene_usd.py#L54) — constant
- [`STINGRAY_SLOT_CHANNELS`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene_usd.py#L68) — constant
- [`STINGRAY_DATA_SLOTS`](blendertk/blendertk/env_utils/maya_bridge/templates/_import_scene_usd.py#L79) — constant
...
- [`import_usd(cmds)`](blendertk/blendertk/env_utils/maya_bridge/templates/_save_scene.py#L93) — Import a USD payload through mayaUsd's file translator;
- [`import_payload(cmds, mel, engine)`](blendertk/blendertk/env_utils/maya_bridge/templates/_save_scene.py#L110) — Run the importer the payload's extension names (FBX or USD).
- [`restore_usd_locators(cmds, engine, new_nodes)`](blendertk/blendertk/env_utils/maya_bridge/templates/_save_scene.py#L117) — Give point-marker Empties their locator shape back (USD carrier) -- the
- [`import_fbx(cmds, mel, engine)`](blendertk/blendertk/env_utils/maya_bridge/templates/_save_scene.py#L144) — Import the payload deterministically;
- [`restore_empty_groups(cmds, engine, new_nodes)`](blendertk/blendertk/env_utils/maya_bridge/templates/_save_scene.py#L176) — Restore imported Empties as the correct Maya node types (see module docstring).
- [`rebuild_materials(engine, new_nodes)`](blendertk/blendertk/env_utils/maya_bridge/templates/_save_scene.py#L204) — Replay the sidecar manifest through mayatk's applier (see module docstring).
- [`rebuild_shots(engine, new_nodes)`](blendertk/blendertk/env_utils/maya_bridge/templates/_save_scene.py#L222) — Rebuild the sent scene's shots from the sidecar's ``shots`` section through
- [`main()`](blendertk/blendertk/env_utils/maya_bridge/templates/_save_scene.py#L237)
- [`BRIDGE_MODES`](blendertk/blendertk/env_utils/maya_bridge/templates/_save_scene.py#L39) — constant
- [`FBX_PATH`](blendertk/blendertk/env_utils/maya_bridge/templates/_save_scene.py#L50) — constant
- [`USD_EXTENSIONS`](blendertk/blendertk/env_utils/maya_bridge/templates/_save_scene.py#L51) — constant
- [`CARRIER`](blendertk/blendertk/env_utils/maya_bridge/templates/_save_scene.py#L52) — constant
- [`OUT_FILE`](blendertk/blendertk/env_utils/maya_bridge/templates/_save_scene.py#L53) — constant
- [`EXTRA_SYS_PATH`](blendertk/blendertk/env_utils/maya_bridge/templates/_save_scene.py#L56) — constant
- [`SHADER_TYPE`](blendertk/blendertk/env_utils/maya_bridge/templates/_save_scene.py#L59) — constant
- [`USD_IMPORT_OPTIONS`](blendertk/blendertk/env_utils/maya_bridge/templates/_save_scene.py#L67) — constant
...
- [`import_fbx()`](blendertk/blendertk/env_utils/maya_bridge/templates/import.py#L72) — Import the payload deterministically;
- [`import_usd()`](blendertk/blendertk/env_utils/maya_bridge/templates/import.py#L102) — Import a USD payload through mayaUsd's file translator;
- [`import_payload()`](blendertk/blendertk/env_utils/maya_bridge/templates/import.py#L123) — Run the importer the payload's extension names (FBX or USD).
- [`restore_usd_locators(new_nodes)`](blendertk/blendertk/env_utils/maya_bridge/templates/import.py#L132) — Give the Empties that mark a point their locator shape back (USD carrier).
- [`restore_empty_groups(new_nodes)`](blendertk/blendertk/env_utils/maya_bridge/templates/import.py#L216) — Restore imported Empties as the CORRECT Maya node types (see module docstring).
- [`rebuild_materials(new_nodes)`](blendertk/blendertk/env_utils/maya_bridge/templates/import.py#L261) — Replay the sidecar manifest through mayatk's applier (see module docstring).
- [`rebuild_shots(new_nodes)`](blendertk/blendertk/env_utils/maya_bridge/templates/import.py#L283) — Rebuild the sent scene's shots from the sidecar's ``shots`` section through
- [`main()`](blendertk/blendertk/env_utils/maya_bridge/templates/import.py#L306)
- [`BRIDGE_MODES`](blendertk/blendertk/env_utils/maya_bridge/templates/import.py#L35) — constant
- [`FBX_PATH`](blendertk/blendertk/env_utils/maya_bridge/templates/import.py#L51) — constant
- [`USD_EXTENSIONS`](blendertk/blendertk/env_utils/maya_bridge/templates/import.py#L52) — constant
- [`CARRIER`](blendertk/blendertk/env_utils/maya_bridge/templates/import.py#L53) — constant
- [`EXTRA_SYS_PATH`](blendertk/blendertk/env_utils/maya_bridge/templates/import.py#L56) — constant
- [`CLEAR_SCENE`](blendertk/blendertk/env_utils/maya_bridge/templates/import.py#L57) — constant
- [`FRAME_VIEW`](blendertk/blendertk/env_utils/maya_bridge/templates/import.py#L58) — constant
- [`SHADER_TYPE`](blendertk/blendertk/env_utils/maya_bridge/templates/import.py#L61) — constant
- [`USD_IMPORT_OPTIONS`](blendertk/blendertk/env_utils/maya_bridge/templates/import.py#L69) — constant
...
- **[`class ReferenceManagerSlots(ptk.LoggingMixin)`](blendertk/blendertk/env_utils/reference_manager.py#L70)** — Switchboard slot wiring for the Reference Manager panel.
...
- **[`class SceneExporter(ptk.LoggingMixin)`](blendertk/blendertk/env_utils/scene_exporter/_scene_exporter.py#L116)**
...
- **[`class SceneExporterSlots(SceneExporter)`](blendertk/blendertk/env_utils/scene_exporter/scene_exporter_slots.py#L39)**
  ...
  - `SceneExporterSlots.export_data_node_init(self, widget) -> None` — Init Export Scene Data Node — a Settings row (``cmb008``) (mirror of
...
- **[`class TaskManager(TaskFactory, _SceneTasksMixin, _TextureTasksMixin, _AnimationTasksMixin, _TaskChecksMixin, _TaskDefinitionsMixin)`](blendertk/blendertk/env_utils/scene_exporter/task_manager.py#L34)** — The export pipeline's tasks and checks, run in the shared order.
  ...
  - `TaskManager.publish_clip_mode(self) -> None` **DEPRECATED (remove in 0.9.0)** — Republish the shot record with this run's Animation Clips mode.
...
- **[`class SceneState`](blendertk/blendertk/env_utils/scene_state.py#L32)** — Section-registry reader of scene state the FBX cannot express.
...
- **[`class ScriptConsole`](blendertk/blendertk/env_utils/script_output.py#L228)** — Singleton orchestrator: capture + native-docked ``uitk.ScriptOutput``, with the
...
- [`USD_EXTENSIONS`](blendertk/blendertk/env_utils/usd.py#L41) — constant
- **[`class UsdUtils(_UsdUtilsInternal)`](blendertk/blendertk/env_utils/usd.py#L148)** — USD import / export over ``bpy.ops`` (mirror of mayatk's ``UsdUtils``).
  - `UsdUtils.is_usd_file(filepath) -> bool` *(static)* — True when *filepath* is a USD layer/package (delegates to pythontk).
  - `UsdUtils.export(filepath=None, objects=None, selection_only=True, frame_range=None, include_hidden=True, **usd_opts)` *(static)* — Export to a USD file — the counterpart of mayatk's ``UsdUtils.export``.
  - `UsdUtils.sampling_frame_range(objects: Optional[List[Any]] = None) -> Optional[Tuple[float, float]]` *(static)* — The frames a USD export is worth sampling, or ``None`` for a static one.
  - `UsdUtils.fold_single_mesh_xforms(filepath: str) -> int` *(static)* — Fold every ``Xform`` whose only child is a ``Mesh`` into one Mesh prim
  - `UsdUtils.sanitize_prim_name(name: str) -> str` *(static)* — *name* as Blender's USD exporter spells the prim (probe-verified on 5.1:
  - `UsdUtils.hidden_objects(objects: Optional[List[Any]] = None) -> List[Any]` *(static)* — The viewport-hidden objects among *objects* (default: every object in
  - `UsdUtils.export_prim_path(obj, root_prim_path: str = '') -> str` *(static)* — The prim path Blender's exporter writes for *obj* (one prim per
  - `UsdUtils.prim_path(obj) -> str` *(static)* — The prim path an IMPORTED *obj* came from: its parent chain with the
  - `UsdUtils.pin_primvar_indices(filepath: str) -> int` *(static)* — Give every indexed primvar a DEFAULT index array, copied from its lone
  - `UsdUtils.mark_skinning_methods(filepath: str, objects: Optional[List[Any]] = None, root_prim_path: str = '') -> int` *(static)* — Stamp each skinned mesh's SKINNING METHOD into the layer, read from its
  - `UsdUtils.mark_container_skeletons(filepath: str) -> int` *(static)* — Mark each Skeleton prim that is only a CONTAINER for its bones with
  - `UsdUtils.mark_invisible(filepath: str, objects: List[Any], root_prim_path: str = '') -> int` *(static)* — Stamp ``visibility = invisible`` on the prims *objects* exported to
  - `UsdUtils.apply_visibility(filepath: str, objects: List[Any]) -> int` *(static)* — Reproduce the layer's visibility on *objects* -- ``hide_viewport`` +
  - `UsdUtils.activate_uv_map(objects: List[Any], name: str = 'map1') -> int` *(static)* — Make the UV map *name* the active and render UV map on every mesh in
  - `UsdUtils.import_scene(filepath: str, **usd_opts) -> List[Any]` *(static)* — Import a hand-off USD layer the way the bridges do: EVERY prim
  - `UsdUtils.import_usd(filepath, **usd_opts)` *(static)* — Import a USD file (wrapper over ``bpy.ops.wm.usd_import``).
  - `UsdUtils.bake_transform_caches(objects: Optional[List[Any]] = None, frame_range: Optional[Tuple[float, float]] = None, clean: bool = True) -> int` *(static)* — Bake every ``TRANSFORM_CACHE`` constraint on *objects* (default: all) into
  - `UsdUtils.honor_reset_xform_stack(usd_path: str, objects: Optional[List[Any]] = None) -> int` *(static)* — Stop imported objects whose prim declares ``!resetXformStack!`` from
  - `UsdUtils.skinning_methods(usd_path: str) -> Dict[str, str]` *(static)* — ``{prim path: skinning method}`` for every prim of the stage that authors
  - `UsdUtils.scene_settings(filepath: str) -> Dict[str, Any]` *(static)* — The time setup a USD stage itself carries, as a (partial) ``scene`` record
...
- **[`class LightUtils(_LightUtilsInternal)`](blendertk/blendertk/light_utils/_light_utils.py#L131)** — Namespace mirror of mayatk's ``light_utils`` (helpers also exposed module-level).
...
- **[`class LightmapBaker(ptk.LoggingMixin)`](blendertk/blendertk/light_utils/lightmap_baker/lightmap_baker.py#L58)** — Orchestrate the Blender lightmap workflow: UV2 -> Cycles bake -> engine export prep.
  ...
  - `LightmapBaker.export_record(cls, ctx: ptk.ExportContext) -> Optional[ptk.Record]` *(class)* — The ``lightmap_metadata`` record for this file, or ``None`` when no
...
- **[`class LightmapBakerSlots(ptk.LoggingMixin, ptk.HelpMixin)`](blendertk/blendertk/light_utils/lightmap_baker/lightmap_baker.py#L1833)** — Switchboard slots for the co-located ``lightmap_baker.ui`` panel.
...
- **[`class EmissiveGroupsSlots(ptk.LoggingMixin, ptk.HelpMixin)`](blendertk/blendertk/mat_utils/emissive_groups.py#L854)** — Switchboard slots for the ``emissive_groups.ui`` panel.
...
- [`ROUNDTRIP`](blendertk/blendertk/mat_utils/marmoset_bridge/_marmoset_engine.py#L68) — constant
...
- [`SLOT_MAP`](blendertk/blendertk/mat_utils/marmoset_bridge/_toolbag_helpers.py#L648) — constant
- [`MODULE_NEUTRAL_FIELDS`](blendertk/blendertk/mat_utils/marmoset_bridge/_toolbag_helpers.py#L671) — constant
- [`SUBROUTINE_FIXES`](blendertk/blendertk/mat_utils/marmoset_bridge/_toolbag_helpers.py#L684) — constant
...
- [`DEFAULT_HOST`](blendertk/blendertk/mat_utils/marmoset_bridge/marmoset_rpc/connection.py#L37) — constant
- [`DEFAULT_PORT`](blendertk/blendertk/mat_utils/marmoset_bridge/marmoset_rpc/connection.py#L38) — constant
...
- [`start_server(port=None, host=None)`](blendertk/blendertk/mat_utils/marmoset_bridge/marmoset_rpc/plugin_src/marmoset_rpc/__init__.py#L53) — Start the RPC server (idempotent).
- [`stop_server()`](blendertk/blendertk/mat_utils/marmoset_bridge/marmoset_rpc/plugin_src/marmoset_rpc/__init__.py#L58) — Shut the server down (tests / hot-reload).
- [`is_running()`](blendertk/blendertk/mat_utils/marmoset_bridge/marmoset_rpc/plugin_src/marmoset_rpc/__init__.py#L63) — True while the server is bound.
- [`autostart()`](blendertk/blendertk/mat_utils/marmoset_bridge/marmoset_rpc/plugin_src/marmoset_rpc/__init__.py#L68) — Start on plugin load, gated to the Toolbag host.
- [`PLUGIN`](blendertk/blendertk/mat_utils/marmoset_bridge/marmoset_rpc/plugin_src/marmoset_rpc/__init__.py#L33) — constant
...
- [`summary()`](blendertk/blendertk/mat_utils/marmoset_bridge/marmoset_rpc/plugin_src/marmoset_rpc/ops/scene_ops.py#L15) — High-level snapshot of the current Toolbag scene.
...
- [`version()`](blendertk/blendertk/mat_utils/marmoset_bridge/marmoset_rpc/plugin_src/marmoset_rpc/ops/system_ops.py#L15) — Toolbag build number (e.g.
...
- [`DEFAULTS`](blendertk/blendertk/mat_utils/marmoset_bridge/template_params.py#L26) — constant
- **[`class TemplateParams`](blendertk/blendertk/mat_utils/marmoset_bridge/template_params.py#L97)** — TemplateParams — module namespace.
...
- [`BRIDGE_MODES`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L22) — constant
- [`MODEL_FILE`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L42) — constant
- [`SOURCE_MODEL_FILE`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L43) — constant
- [`PAIRS_FILE`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L44) — constant
- [`MANIFEST_FILE`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L45) — constant
- [`OUTPUT_DIR`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L46) — constant
- [`SAVE_PATH`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L47) — constant
- [`SHOULD_QUIT`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L48) — constant
- [`BAKE_SIZE`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L58) — constant
- [`BAKE_SAMPLES`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L59) — constant
- [`BAKE_PADDING`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L60) — constant
- [`BAKE_BITS`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L61) — constant
- [`OUTPUT_FORMAT`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L62) — constant
- [`MAP_NORMAL`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L65) — constant
- [`MAP_AO`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L66) — constant
- [`MAP_CURVATURE`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L67) — constant
- [`MAP_THICKNESS`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L68) — constant
- [`MAP_POSITION`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L69) — constant
- [`MAP_MATID`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L70) — constant
- [`MAP_ALBEDO`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L71) — constant
- [`MAP_ROUGHNESS`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L72) — constant
- [`MAP_METALNESS`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L73) — constant
- [`MAP_EMISSIVE`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L74) — constant
- [`HIGH_SUFFIX`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L77) — constant
- [`LOW_SUFFIX`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L78) — constant
- [`SUFFIX_INCLUDE_CHILDREN`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L79) — constant
- [`CAGE_OFFSET`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L80) — constant
- [`AUTO_CAGE`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L81) — constant
- [`IGNORE_BACKFACES`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L82) — constant
- [`CAGE_STANDOFFS`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L93) — constant
- [`CAGE_HOST_DIAGONAL`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L94) — constant
- [`CAGE_REACH_FACTOR`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L103) — constant
- [`AUTO_CAGE_MARGIN`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L108) — constant
- [`AUTO_CAGE_FLOOR`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L126) — constant
- [`AUTO_CAGE_BOUNDS_FLOOR`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L127) — constant
- [`AUTO_CAGE_CEILING`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L128) — constant
- [`AUTO_CAGE_GAP_MARGIN`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/bake.py#L132) — constant
...
- [`BRIDGE_MODES`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/import.py#L12) — constant
- [`MODEL_FILE`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/import.py#L25) — constant
- [`MANIFEST_FILE`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/import.py#L26) — constant
- [`SAVE_PATH`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/import.py#L27) — constant
- [`SHOULD_QUIT`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/import.py#L28) — constant
...
- [`BRIDGE_MODES`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/lookdev.py#L12) — constant
- [`MODEL_FILE`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/lookdev.py#L25) — constant
- [`MANIFEST_FILE`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/lookdev.py#L26) — constant
- [`SAVE_PATH`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/lookdev.py#L27) — constant
- [`SHOULD_QUIT`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/lookdev.py#L28) — constant
- [`SKY_PRESET`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/lookdev.py#L30) — constant
- [`FRAME_SELECTION`](blendertk/blendertk/mat_utils/marmoset_bridge/templates/lookdev.py#L31) — constant
...
- **[`class RenderEffects(ptk.LoggingMixin)`](blendertk/blendertk/mat_utils/render_opacity/render_effects.py#L32)** — Per-object render-effect channels: the keyable ``opacity`` prop (mirrored
  ...
  - `RenderEffects.export_record(cls, ctx: ptk.ExportContext) -> Optional[ptk.Record]` *(class)* — The ``visibility_tracks`` record for this file, or ``None`` when it
...
- [`ROUNDTRIP`](blendertk/blendertk/mat_utils/substance_bridge/_substance_bridge.py#L58) — constant
- [`TARGET_AUTO`](blendertk/blendertk/mat_utils/substance_bridge/_substance_bridge.py#L70) — constant
- [`TARGET_NEW`](blendertk/blendertk/mat_utils/substance_bridge/_substance_bridge.py#L71) — constant
- [`TARGET_CURRENT`](blendertk/blendertk/mat_utils/substance_bridge/_substance_bridge.py#L72) — constant
- **[`class HighPolySet`](blendertk/blendertk/mat_utils/substance_bridge/_substance_bridge.py#L163)** — The scene's high-poly bake source, stored as a stamped Collection.
...
- **[`class SubstanceBridge(ptk.HandoffBridge)`](blendertk/blendertk/mat_utils/substance_bridge/_substance_bridge.py#L273)** — Export Blender selection to Substance Painter via a chosen template.
...
- [`start_server(port=None, host=None)`](blendertk/blendertk/mat_utils/substance_bridge/substance_rpc/plugin_src/substance_rpc/__init__.py#L54) — Start the RPC server (idempotent).
- [`stop_server()`](blendertk/blendertk/mat_utils/substance_bridge/substance_rpc/plugin_src/substance_rpc/__init__.py#L59) — Shut the server down (close_plugin hook / tests / hot-reload).
- [`is_running()`](blendertk/blendertk/mat_utils/substance_bridge/substance_rpc/plugin_src/substance_rpc/__init__.py#L64) — True while the server is bound.
- [`autostart()`](blendertk/blendertk/mat_utils/substance_bridge/substance_rpc/plugin_src/substance_rpc/__init__.py#L69) — Start on plugin load, gated to the Painter host.
- [`start_plugin()`](blendertk/blendertk/mat_utils/substance_bridge/substance_rpc/plugin_src/substance_rpc/__init__.py#L74) — Painter lifecycle hook: start the RPC server (idempotent).
- [`close_plugin()`](blendertk/blendertk/mat_utils/substance_bridge/substance_rpc/plugin_src/substance_rpc/__init__.py#L81) — Painter lifecycle hook: shut the RPC server down.
- [`PLUGIN`](blendertk/blendertk/mat_utils/substance_bridge/substance_rpc/plugin_src/substance_rpc/__init__.py#L34) — constant
...
- [`project_info()`](blendertk/blendertk/mat_utils/substance_bridge/substance_rpc/plugin_src/substance_rpc/ops/project_ops.py#L28) — Return ``{is_open, file_path, mesh_path, needs_saving}`` (best-effort).
- [`mesh_reload(mesh_path='', preserve_strokes=True, import_cameras=False)`](blendertk/blendertk/mat_utils/substance_bridge/substance_rpc/plugin_src/substance_rpc/ops/project_ops.py#L61) — Reload the open project's mesh from *mesh_path* (async).
- [`mesh_reload_status()`](blendertk/blendertk/mat_utils/substance_bridge/substance_rpc/plugin_src/substance_rpc/ops/project_ops.py#L103) — Outcome of the last ``mesh.reload``: pending / success / failure.
...
- [`version()`](blendertk/blendertk/mat_utils/substance_bridge/substance_rpc/plugin_src/substance_rpc/ops/system_ops.py#L19) — Return Painter + plugin API version info (best-effort).
- [`eval_python(script='')`](blendertk/blendertk/mat_utils/substance_bridge/substance_rpc/plugin_src/substance_rpc/ops/system_ops.py#L39) — Exec *script* (Python source) inside Painter's interpreter.
- [`js_evaluate(script='')`](blendertk/blendertk/mat_utils/substance_bridge/substance_rpc/plugin_src/substance_rpc/ops/system_ops.py#L61) — Evaluate *script* in Painter's JavaScript engine (``alg.*`` API).
...
- [`BRIDGE_MODES`](blendertk/blendertk/mat_utils/substance_bridge/templates/bake_lighting.py#L52) — constant
- [`LAUNCH_ARGS`](blendertk/blendertk/mat_utils/substance_bridge/templates/bake_lighting.py#L66) — constant
- [`RPC_SCRIPT`](blendertk/blendertk/mat_utils/substance_bridge/templates/bake_lighting.py#L76) — constant
- [`BUILD_MANIFEST`](blendertk/blendertk/mat_utils/substance_bridge/templates/bake_lighting.py#L102) — constant
- [`FBX_OPTIONS`](blendertk/blendertk/mat_utils/substance_bridge/templates/bake_lighting.py#L107) — constant
- [`TARGET_INSTANCE`](blendertk/blendertk/mat_utils/substance_bridge/templates/bake_lighting.py#L112) — constant
...
- [`BRIDGE_MODES`](blendertk/blendertk/mat_utils/substance_bridge/templates/import.py#L14) — constant
- [`LAUNCH_ARGS`](blendertk/blendertk/mat_utils/substance_bridge/templates/import.py#L47) — constant
- [`RPC_SCRIPT`](blendertk/blendertk/mat_utils/substance_bridge/templates/import.py#L53) — constant
- [`BUILD_MANIFEST`](blendertk/blendertk/mat_utils/substance_bridge/templates/import.py#L60) — constant
- [`FBX_OPTIONS`](blendertk/blendertk/mat_utils/substance_bridge/templates/import.py#L64) — constant
- [`TARGET_INSTANCE`](blendertk/blendertk/mat_utils/substance_bridge/templates/import.py#L70) — constant
...
- [`BRIDGE_MODES`](blendertk/blendertk/mat_utils/substance_bridge/templates/reimport.py#L29) — constant
- [`LAUNCH_ARGS`](blendertk/blendertk/mat_utils/substance_bridge/templates/reimport.py#L45) — constant
- [`RPC_OPS`](blendertk/blendertk/mat_utils/substance_bridge/templates/reimport.py#L50) — constant
- [`RPC_SCRIPT`](blendertk/blendertk/mat_utils/substance_bridge/templates/reimport.py#L61) — constant
- [`BUILD_MANIFEST`](blendertk/blendertk/mat_utils/substance_bridge/templates/reimport.py#L63) — constant
- [`REUSE_RECORDED_EXPORT`](blendertk/blendertk/mat_utils/substance_bridge/templates/reimport.py#L68) — constant
- [`TARGET_INSTANCE`](blendertk/blendertk/mat_utils/substance_bridge/templates/reimport.py#L74) — constant
- [`NO_CONNECTION_HINT`](blendertk/blendertk/mat_utils/substance_bridge/templates/reimport.py#L76) — constant
...
- [`BRIDGE_MODES`](blendertk/blendertk/mat_utils/substance_bridge/templates/render.py#L25) — constant
- [`LAUNCH_ARGS`](blendertk/blendertk/mat_utils/substance_bridge/templates/render.py#L28) — constant
- [`EXPORT_FBX`](blendertk/blendertk/mat_utils/substance_bridge/templates/render.py#L33) — constant
- [`FBX_OPTIONS`](blendertk/blendertk/mat_utils/substance_bridge/templates/render.py#L34) — constant
- [`RPC_SCRIPT`](blendertk/blendertk/mat_utils/substance_bridge/templates/render.py#L49) — constant
- [`BUILD_MANIFEST`](blendertk/blendertk/mat_utils/substance_bridge/templates/render.py#L58) — constant
- [`TARGET_INSTANCE`](blendertk/blendertk/mat_utils/substance_bridge/templates/render.py#L61) — constant
...
- **[`class Channels`](blendertk/blendertk/node_utils/attributes/channels/_channels.py#L23)** — Blender attribute query / mutation logic.
...
- **[`class DataNodes(ptk.SceneStoreBase)`](blendertk/blendertk/node_utils/data_nodes.py#L39)** — Blender's scene store: the ``data_internal`` scene property group and
  ...
  - `DataNodes.get_export_nodes(cls) -> List[Any]` *(class)* — Every ``data_export`` carrier in the file, the file's own first
  - `DataNodes.ensure_export(cls)` *(class)* — Get or create the ``data_export`` Empty.
  - `DataNodes.read(cls, scope: ptk.Scope, key: str) -> Optional[str]` *(class)* — The string channel *key* in *scope*, or ``None`` when the carrier,
  - `DataNodes.write(cls, scope: ptk.Scope, key: str, text: Optional[str]) -> Optional[str]` *(class)* — Store *text* on *key* in *scope*.
  - `DataNodes.values(cls, scope: ptk.Scope) -> Dict[str, Any]` *(class)* — Every property on the carrier of *scope*, strings and otherwise,
  ...
  - `DataNodes.set_internal_string(cls, key, value)` *(class)* **DEPRECATED**
  - `DataNodes.get_internal_string(cls, key)` *(class)* **DEPRECATED**
  - `DataNodes.set_internal_json(cls, key, payload)` *(class)* **DEPRECATED**
  - `DataNodes.get_internal_json(cls, key, default=None)` *(class)* **DEPRECATED**
  - `DataNodes.set_export_string(cls, key, value)` *(class)* **DEPRECATED**
  - `DataNodes.get_export_string(cls, key)` *(class)* **DEPRECATED**
  - `DataNodes.set_export_json(cls, key, payload)` *(class)* **DEPRECATED**
...
- **[`class RigUtils`](blendertk/blendertk/rig_utils/_rig_utils.py#L21)** — Constraint / driver / handle / grouping / armature helpers shared by the procedural rigs.
  ...
  - `RigUtils.set_bone_heads(armature, heads)` *(static)* — Move bones by NAME — ``{bone: (x, y, z)}``, the new HEAD in armature space.
  - `RigUtils.set_bone_lengths(armature, lengths)` *(static)* — Resize bones by NAME — ``{bone: length}``, in the armature's own units.
...
<a id="rig_utils--rig_graph_build"></a>
### `rig_utils/rig_graph_build.py`
...
- **[`class RigGraphBuilder(_RigGraphBuilderInternal, ptk.HelpMixin)`](blendertk/blendertk/rig_utils/rig_graph_build.py#L557)** — Plan a RigGraph against Blender and build what the plan allows.
  ...
  - `RigGraphBuilder.up_axis() -> str` *(static)* — Blender is Z-up.
  - `RigGraphBuilder.sample_world(self, node_id: str, frame: int) -> Optional[Tuple[float, float, float]]` — The world position of what *node_id* names at *frame* -- an object's
  - `RigGraphBuilder.commit(self, record_id: str) -> int` — A verified record owns its channels: delete the payload's muted keys
  - `RigGraphBuilder.remove(self, record_id: str) -> int` — Take back everything :meth:`build` created for *record_id* -- the
  - `RigGraphBuilder.build(self, graph: Dict[str, Any], imported: Sequence[Any], is_usd: bool = False) -> Dict[str, Any]` — Plan *graph* against :meth:`capability` and build what it allows.
  - `RigGraphBuilder.evaluable(self)` — Yield with every object a built record touches evaluable at any

<a id="rig_utils--rig_graph_extract"></a>
### `rig_utils/rig_graph_extract.py`
...
- **[`class ShadowRig(ptk.LoggingMixin)`](blendertk/blendertk/rig_utils/shadow_rig.py#L78)** — Projected-shadow rig for engine export (mirror of mayatk's ``ShadowRig``).
  ...
  - `ShadowRig.setup_drivers(self)` — Build the model's driver chain (group -> contact -> plane), the material's opacity
  - `ShadowRig.plane_is_live(cls, plane)` *(class)* — Does *plane* still carry its drivers (not baked)?
  - `ShadowRig.plane_is_baked(cls, plane)` *(class)* — Does *plane* carry baked keys (an action) on its channels?
  ...
  - `ShadowRig.planes_for_nodes(cls, objects)` *(class)* — Shadow planes the given objects touch: the planes themselves (or their
  - `ShadowRig.for_node(cls, obj)` *(class)* — The rig *obj* belongs to (a plane, its group, a target, the source, the contact),
  - `ShadowRig.for_nodes(cls, objects)` *(class)* — Distinct rigs the *objects* touch (see :meth:`for_node`).
  - `ShadowRig.bake_planes(cls, planes=None, start=None, end=None)` *(class)* — Bake shadow planes' driven channels — transform AND the ``opacity`` fade — to
  - `ShadowRig.unbake_planes(cls, planes=None)` *(class)* — Restore the live drivers on baked shadow planes — the reverse of
  ...
  - `ShadowRig.export_record(cls, ctx)` *(class)* — The ``shadow_metadata`` record for this file, or ``None`` when it
  ...
  - `ShadowRig.plane_record(cls, plane)` *(class)* — One plane's ``shadow_metadata`` v2 record (the engine contract in
...
- **[`class ShadowRigSlots(ptk.LoggingMixin)`](blendertk/blendertk/rig_utils/shadow_rig.py#L2716)** — Switchboard slot wiring for the Shadow Rig panel.
  ...
  - `ShadowRigSlots.b009(self)` — Delete Rig: tear down the rig(s) the selection touches.
  - `ShadowRigSlots.b010(self)` — Pack Atlas: pack or repack every shadow rig's tiles into the file's
  - `ShadowRigSlots.perform_operation(self, objects)` — Build one shadow rig per source for the selected target(s).
...
- [`EDITOR_TYPES`](blendertk/blendertk/ui_utils/_ui_utils.py#L16) — constant
- **[`class UiUtils(_UiUtilsInternal)`](blendertk/blendertk/ui_utils/_ui_utils.py#L149)** — Namespace mirror (helpers also exposed module-level).
  ...
  - `UiUtils.reveal_in_outliner(objects)` *(static)* — Select *objects* (names or Objects) and scroll the Outliner to the active one.
...
- **[`class BlenderWindow`](blendertk/blendertk/ui_utils/blender_window.py#L34)** — Static win32 helpers for GHOST-window enumeration, geometry, embedding, ownership.
...
- **[`class BlenderCancelProvider(CancelProvider)`](blendertk/blendertk/ui_utils/cancel_provider.py#L50)** — Blender host strategy for :class:`uitk.CancelManager`.
...
- [`ICON_COLOR`](blendertk/blendertk/ui_utils/node_icons.py#L45) — constant
- **[`class NodeIcons`](blendertk/blendertk/ui_utils/node_icons.py#L48)** — Resolve Blender object-type icons as Qt QIcons.
  - `NodeIcons.icon_name_for_type(obj_type: str) -> Optional[str]` *(static)* — Return the uitk icon name for a Blender ``Object.type`` (or ``None``).
  - `NodeIcons.icon_name_for_node(obj_name: str) -> Optional[str]` *(static)* — Return the icon name for the named scene object.
  - `NodeIcons.get_icon(obj_name: str, size: int = 16)` *(static)* — Return a ``QIcon`` for a Blender object, or ``None`` if unavailable.
  - `NodeIcons.get_pixmap(obj_name: str, size: int = 16)` *(static)* — Return a ``QPixmap`` for a Blender object scaled to *size*, or ``None``.
...
- [`STYLES_DIR`](blendertk/blendertk/ui_utils/style_setter/_style_setter.py#L49) — constant
...
<a id="ui_utils--ui_state"></a>
### `ui_utils/ui_state.py`

Persist Blender's per-session UI visibility state across sessions (``btk.UiState``).

- **[`class UiState(_UiStateInternal)`](blendertk/blendertk/ui_utils/ui_state.py#L125)** — Session-persistent UI visibility state.
  - `UiState.state_path(cls) -> Optional[str]` *(class)* — The sidecar path (Blender's user ``CONFIG`` dir), or None when unresolvable.
  - `UiState.load(cls) -> Dict` *(class)* — The saved state (``{"version", "workspaces": {name: {"hidden", "spaces"}}}``);
  - `UiState.save(cls, state: Optional[Dict] = None) -> bool` *(class)* — Write ``state`` (default: the live class state) atomically — the tick writes
  - `UiState.clear(cls) -> bool` *(class)* — Forget the saved state (delete the sidecar) — the "reset to file defaults" path.
  - `UiState.snapshot_spaces(cls, screen) -> Dict[str, Dict[str, bool]]` *(class)* — ``{ui_type: {flag: value}}`` read from the largest area of each ``ui_type``.
  - `UiState.snapshot_workspace(cls, window, saved: Optional[Dict] = None) -> Dict` *(class)* — The workspace entry for ``window``'s active workspace: ``hidden`` merged per
...
- **[`class AutoUnwrapResult`](blendertk/blendertk/uv_utils/_auto_unwrap.py#L19)** — Per-object outcome of an :meth:`auto_unwrap` run.
...
- **[`class RizomBridgeSlots(BlenderBridgeSlotsBase)`](blendertk/blendertk/uv_utils/rizom_bridge/rizom_bridge_slots.py#L74)** — Slots wired to ``rizom_bridge.ui`` via :class:`BlenderBridgeSlotsBase`.
